...
class Channel {
 ...
 public:
  ...
  void Add(shared_ptr<Transceiver> transceiver) {
...
void Channel::Send(double t, const Transmission &transm,
  ...
  for (const auto &transceiver : transceivers_) {
    if (transceiver.get() != &sender) {
      Scheduler::Schedule(t + delay_, [transceiver, transm](double t) {
        transceiver->Receive(t, transm);
      });
    }
  }
}
...
class RingAgent {
 ...
 public:
  RingAgent()
      : id_(id_counter_++), transceivers_(vector<shared_ptr<Transceiver>>()) {}
