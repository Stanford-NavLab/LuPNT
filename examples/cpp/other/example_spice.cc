...
int main() {
  ...
  Real t_tt = 0;
  Real t_tai = ConvertTime(t_tt, TimeSys::TT, TimeSys::TAI);
  Real t_tdb = ConvertTime(t_tt, TimeSys::TT, TimeSys::TDB);

  int prec = 3;
  std::string str = TAItoStringUTC(t_tai, prec);
  std::cout << "TAI: " << t_tai << std::endl;
  std::cout << "TDB: " << t_tdb << std::endl;
  std::cout << "TT: " << t_tt << std::endl;
  ...
  auto func = [](const auto& t_tai) {
    return GetBodyPosVel(t_tai, NaifId::MOON, NaifId::EARTH, Frame::GCRF);
  };
