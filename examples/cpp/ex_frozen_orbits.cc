...
int main() {
  ...
  if (config.plot_case0) {
    ...
    plt::title("Case 0: Moon orbit in CI frame");
    plt::hold(true);
    PlotBody(MOON);
  ...
  if (config.plot_case1) {
    ...
    plt::title("Case 1: Moon orbit in OP frame");
    plt::hold(true);
    PlotBody(EARTH);
  ...
  MatX6 rv_case1_op;
  if (config.recompute_case1 || !file.exist("/case1/rv_case1_op")) {
  ...
  if (config.plot_case1) {
    ...
    plt::title("Case 1: Orbital elements in OP frame");
    plt::hold(true);
    for (int i = 0; i < 6; i++) {
      plt::subplot(3, 2, i);
    ...
    plt::title("Case 1: Satellite orbit in OP frame");
    plt::hold(true);
    PlotBody(MOON);
    for (int i = 0; i < n_plot; i++) {
  ...
  // Propagate
  MatX6 rv_case2_ci;
  if (config.recompute_case2 || !file.exist("/case2/rv_case2_ci")) {
    cout << endl << "Propagating" << endl;
  ...
  if (config.plot_case2) {
    ...
    plt::title("Case 2: Satellite orbit in OP frame");
    plt::hold(true);
    PlotBody(MOON);
    for (int i = 0; i < n_plot; i++) {
    ...
    // Plot orbital elements
    auto fig22 = plt::figure(true);
    plt::title("Case 2: Orbital elements in OP frame");
    plt::hold(true);
    for (int i = 0; i < 6; i++) {
      plt::subplot(3, 2, i);
      VecX x = tspan / SECS_DAY;
  ...
  // Propagate
  MatX6 rv_case3_ci;
  if (config.recompute_case3 || !file.exist("/case3/rv_case3_ci")) {
    cout << endl << "Propagating" << endl;
    rv_case3_ci = dyn_case3.Propagate(rv0_ci, t0, tfs, true);
    dump(file, "/case3/rv_case3_ci", rv_case3_ci.cast<double>());
  } else {
  ...
  if (config.plot_case4) {
    ...
    auto fig41 = plt::figure(true);
    plt::title("Case 4: Satellite orbit in ME frame");
    plt::hold(true);
    PlotBody(MOON);
    for (int i = 0; i < n_plot; i++) {
    ...
    // Plot orbital elements
    auto fig42 = plt::figure(true);
    plt::title("Case 4: Orbital elements in ME frame");
    plt::hold(true);
    for (int i = 0; i < 6; i++) {
      plt::subplot(3, 2, i);
      VecX x = tspan / SECS_DAY;
  ...
  int e_idx = 1, w_idx = 4;
  VecX e_vec, w_vec;
  std::array<double, 2> xlims = {70, 110};
  std::array<double, 2> ylims = {0.5, 0.75};
