...
namespace lupnt {
  ...
  struct EphemerisFitOptions {
    ...
    /// inertial frame (e.g. `Frame::MOON_CI`, the default) the two-body baseline
    /// is a standard osculating Kepler orbit. For the rotating Moon-fixed
    /// principal-axis frame (`Frame::MOON_PA`) the baseline is propagated in that
    /// rotating frame -- the ascending node drifts at the Moon's rotation rate and
    ...
    /// Number of Fourier terms (harmonics of the argument of latitude u) appended
    /// to the Chebyshev position-residual model: term h adds a cos(h*u), sin(h*u)
    /// pair per axis, capturing once/twice/... -per-orbit periodic residuals. 0
    /// (the default) is a pure Chebyshev model. Requires `use_keplerian_baseline`
    /// (the argument of latitude comes from the two-body baseline orbit).
