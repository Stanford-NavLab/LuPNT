...
namespace lupnt {
  ...
  struct IslOdtsAppParams {
    ...
    // One-way anchor Doppler (pseudorange-rate) measurement. When enabled, each staged
    // anchor also contributes a range-rate row, adding velocity + clock-drift observability.
    bool include_anchor_doppler = false;
    double anchor_doppler_sigma_mps = 1.0e-3;

    // Two-way time-transfer (crosslink clock-difference) measurement. When enabled, each
    // crosslink also carries a range-equivalent clock-bias difference row that makes the
    // satellites' relative clocks observable (the two-way *range* alone is clock-free).
    bool include_time_transfer = false;
    double time_transfer_sigma_m = 1.0;

    // Two-way frequency-transfer (crosslink clock-drift difference) measurement: the rate
    // companion to the time transfer, making relative clock drift directly observable.
  ...
  struct IslOdtsMeasurementEpoch {
    ...
    // Two-way time-transfer (range-equivalent clock-bias difference) per crosslink, in the
    // same link order. Consumed only when IslOdtsAppParams::include_time_transfer is set.
    VecXd crosslink_time_transfer_m;  // [n_links]

    // Two-way frequency-transfer (range-rate-equivalent clock-drift difference) per
    // crosslink. Consumed only when IslOdtsAppParams::include_frequency_transfer is set.
    VecXd crosslink_frequency_transfer_mps;  // [n_links]
    ...
    // Optional one-way anchor Doppler (pseudorange-rate), consumed only when
    // IslOdtsAppParams::include_anchor_doppler is set. anchor_vel_mci parallels
    // anchor_pos_mci; anchor_doppler_mps holds the observed range-rates [m/s].
