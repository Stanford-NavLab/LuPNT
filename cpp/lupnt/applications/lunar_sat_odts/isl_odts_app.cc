...
namespace lupnt {
  ...
  void IslOdtsApp::Step(Real t) {
    ...
    const bool tt = params_.include_time_transfer;
    const int n_tt = tt ? n_links : 0;
    const bool ft = params_.include_frequency_transfer;
    const int n_ft = ft ? n_links : 0;
    const bool dopp = params_.include_anchor_doppler && !staged_.anchor_vel_mci.empty();
    const int n_ad = dopp ? n_anchor : 0;
    ...
    meas_cfg.anchor_vel_mci = staged_.anchor_vel_mci;
    ...
    meas_cfg.include_anchor_doppler = dopp;
    meas_cfg.sigma_anchor_doppler_mps = params_.anchor_doppler_sigma_mps;
    meas_cfg.include_time_transfer = tt;
    meas_cfg.sigma_time_transfer_m = params_.time_transfer_sigma_m;
    meas_cfg.include_frequency_transfer = ft;
    meas_cfg.sigma_frequency_transfer_mps = params_.frequency_transfer_sigma_mps;
    filter_->SetMeasurementFunction(IslCrosslinkMeasurement(meas_cfg).CreateFunction());

    VecXd y_obs(2 * n_links + n_anchor + n_ad + n_tt + n_ft);
    ...
    for (int a = 0; a < n_ad; ++a)
      y_obs(2 * n_links + n_anchor + a) = staged_.anchor_doppler_mps(a);
    for (int i = 0; i < n_tt; ++i)
      y_obs(2 * n_links + n_anchor + n_ad + i) = staged_.crosslink_time_transfer_m(i);
    for (int i = 0; i < n_ft; ++i)
      y_obs(2 * n_links + n_anchor + n_ad + n_tt + i) = staged_.crosslink_frequency_transfer_mps(i);
