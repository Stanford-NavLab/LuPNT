...
namespace lupnt {
  ...
  /// iterative batch (weighted least-squares) filter in `lupnt/filters/batch_filter.h`.
  /// The batch design matrix is a numerical (finite-difference) Jacobian of the
  /// analytic range/range-rate observation model (in the style of
  /// `GnssMeasurement::Compute`), built by re-propagating the epoch state with
