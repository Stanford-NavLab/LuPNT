...
C 2012.01 12/12/11 put FMODIP_2020 in EXTERNAL; cgn_lon -> cgm_lon
...
C 2020.25 12/04/24 Omit F1 if NmF1 < XE2_2020(HEF)
...
C
C
       SUBROUTINE IRI_SUB_2020(JF,JMAG,ALATI,ALONG,IYYYY,MMDD,DHOUR,
...
c      CHARACTER FILNAM*53
     ...
     &   /cotec/hnea,hpp
      EXTERNAL
     &XE1_2020,XE2_2020,XE3_1_2020,XE4_1_2020,XE5_2020,XE6_2020,FMODIP_2
     &020
...
c
        ...
        if(MMDD.lt.0) then
                DAYNR=-MMDD
                call MODA_2020(1,iyear,MONTH,IDAY,DAYNR,nrdaym)
        else
                MONTH=MMDD/100
                IDAY=MMDD-MONTH*100
                call MODA_2020(0,iyear,MONTH,IDAY,DAYNR,nrdaym)
        endif
...
C
        ...
        ELSE
           ...
           LONGI=ALONG
        ENDIF
        CALL GEODIP_2020(IYEAR,LATI,LONGI,MLAT,MLONG,JMAG)

        if((iyear.ne.iyearo).or.(
     &daynr.ne.idaynro)) CALL FELDCOF_2020(RYEAR)

        if(jf(18)) then
        	call igrf_dip_2020(lati,longi,ryear,300.0,dec,dip,magbr,modip)
        else
        	CALL
     &FIELDG_2020(LATI,LONGI,300.0,XMA,YMA,ZMA,BET,DIP,DEC,MODIP)
...
c
        ...
        if(jf(3).and.(.not.jf(6))) then
           call igrf_sub_2020(lati,longi,ryear,600.0,fl,icode,dipl,babs)
           if(fl.gt.10.) fl=10.
           invdip=INVDPC_2020(FL,DIMO,BABS,DIPL)
	   endif
        if((jf(2).and.(.not.jf(23))).or.(jf(2).and.jf(48))) then
           call igrf_sub_2020(lati,longi,ryear,600.0,fl,icode,dipl,babs)
           if(fl.gt.10.) fl=10.
      	   invdip_old=INVDPC_OLD_2020(FL,DIMO,BABS,DIPL)
	   endif
...
c
        ...
        ENDIF
        CALL CLCMLT_2020(IYEAR,DAYNR,HOURUT,LATI,LONGI,XMLT)
c
...
c        if(jf(47).and.(abslat.gt.25.0)) then
	   ...
	   DAT(1,1)=lati
	   DAT(2,1)=longi
           call GEOCGM01_2020(1,IYEAR,height_center,DAT,PLA,PLO)
c           cgm_lat=DAT(3,1)
c           cgm_lon=DAT(4,1)
...
5592    continue
     ...
     &		.and..not.f107_81in.and..not.f107_81ino) goto 2910
        ...
        call tcon_2020(iyear,month,iday,daynr,rzar,arig,ttt,nmonth)
...
C observed (at the ground) value.
        ...
        f10781=cov
        f107365=cov
        call APF_ONLY_2020(iyear,month,iday,F107_daily,F107PD,F107_81,
...
2910    continue
        CALL SOCO_2020(daynr,HOUR,LATI,LONGI,80.,
     &SUNDEC,XHI1,SAX80,SUX80)
        CALL SOCO_2020(daynr,HOUR,LATI,LONGI,110.,
     &SUD1,XHI2,SAX110,SUX110)
        CALL SOCO_2020(daynr,HOUR,LATI,LONGI,200.,
     &SUD1,XHI3,SAX200,SUX200)
        CALL SOCO_2020(daynr,HOUR,LATI,LONGI,300.,
     &SUD1,XHI4,SAX300,SUX300)
        CALL SOCO_2020(daynr,12.0,LATI,LONGI,110.,
     &SUNDE1,XHINON,SAX1,SUX1)
        CALL SOCO_2020(daynr,12.0,LATI,LONGI,200.,
     &SUNDE2,XHINON2,SAX2,SUX2)
        DNIGHT=.FALSE.
...
c
        IF(FOEIN) THEN
          ...
          NME=ANME
        ELSE
          FOE=FOEEDI_2020(COV,XHI2,XHINON,ABSLAT)
          NME=1.24E10*FOE*FOE
        ENDIF
        IF(HMEIN) THEN
          HME=AHME
...
c
        call apf_2020(isdate,hourut,indap)

        if(jf(45)) then
           Kpsum=0
           do i=6,13
              xxkp=ckp_2020(indap(i))
              Kpsum=Kpsum+xxkp
              enddo
           isoma=1
           if(jf(46)) isoma=0
           ...
           CALL ESPROB_2020(isoma,isoma,INVDIP_OLD,MLLOO,XMLT,
...
4292    zfof2  =  FOUT_2020(MODIP,LATI,LONGI,HOURUT,FF0)
        fof2n  =  FOUT_2020(MODIP,LATI,LONGI,HOURUT,FF0N)
        zm3000 = XMOUT_2020(MODIP,LATI,LONGI,HOURUT,XM0)
        xm300n = XMOUT_2020(MODIP,LATI,LONGI,HOURUT,XM0N)
        midm=15
...
c
        if(fstorm_on.and.(indap(1).gt.-1)) then
           icoord=1
           kut=int(hourut)
           call STORM_2020(indap,lati,longi,icoord,cglat,kut,
     &          daynr,stormcorr)
...
c
        if(estorm_on.and.(index_3h_ap.gt.-1)) then
           estormcor=STORME_AP_2020(DAYNR,MLAT,index_3h_ap*1.0)
...
c           zmlt=cgm_mlt
           ...
           call auroral_boundary_2020(xkp,zmlt,cgmlat,ab_mlat)
           endif
c
...
c
	   ...
	   FLON=LONGI+15.*hourut
           ...
           X11=-90
           X22=90
           FX11=fmodip_2020(x11)
           FX22=fmodip_2020(x22)
        CALL
     &REGFA1_2020(X11,X22,FX11,FX22,0.001,MODIP,FMODIP_2020,SCHALT,XRLAT
     &)
           IF(SCHALT) THEN
              XRLAT=LATI
...
c if jf(36)=false then foF2_storm in hmF2 formula
              ...
              HMF2=HMF2ED_2020(MAGBR,RSSN,RATF,XM3000)
           ELSE
              HMF2=AHMF2
...
c AMTB digisonde model
 	   CALL SHAMDHMF2_2020(RLAT,FLON,ZMONTH,RSSN,HMF2)
           goto 9917
           endif

c SHUBIN-COSMIC model
	CALL model_hmF2_2020(iday,month,hourut,modip,
     &	   longi,F10781,HMF2)
...
c
      ...
      if(itopn.eq.1.or.itopn.eq.3) then
        zmp111 = EPLA_2020(modip,10.0,0.0)
        zmp222 = EPLA_2020(modip,19.0,0.0)
        r2n = -0.84 - 1.60 * zmp111
        r2d = -0.84 - 0.64 * zmp111
        x1n = 230. -  700. * zmp222
        x1d = 550. - 1900. * zmp222
        r2 = HPOL_2020(HOUR,r2d,r2n,SAX300,SUX300,1.,1.)
        x1 = HPOL_2020(HOUR,x1d,x1n,SAX300,SUX300,1.,1.)
        hcor1 = hmF2 + x1
c        x12 = 1500. - x1
c        tc3 = r2 / x12
...
c
        ...
        if(jf(49)) then
          xnepp =ohzden_2020(xlpp,ppmlat)
          xnemid=ohzden_2020(xlmid,ppmlat)
          xneppo=ohzden_2020(xlppo,ppmlat)
          xnept =ohzden_2020(xlpt,ppmlat)
        else
          xnepp =gallden_2020(xlpp,daynr,rssn)
          xnemid=gallden_2020(xlmid,daynr,rssn)
          xneppo=gallden_2020(xlppo,daynr,rssn)
          xnept =gallden_2020(xlpt,daynr,rssn)
        endif
...
c
        ...
        tcor1=0.0
        CALL SOCO_2020(daynr,HOUR,LATI,LONGI,hmid,SUC,zxz,sap,sup)
        TCOR2 = TCOR2CAL_2020(hmid,hour,modip,pf107,sap,sup)
        znemid = log(XE_1_2020(hmid)/nmf2s)
        palogne(3)=palogne(3)-znemid
        CALL SOCO_2020(daynr,HOUR,LATI,LONGI,hpp,SUC,zxz,sap,sup)
        TCOR2 = TCOR2CAL_2020(hpp,hour,modip,pf107,sap,sup)
        znepp = log(XE_1_2020(hpp)/nmf2s)
        palogne(4)=palogne(4)-znepp
        CALL SOCO_2020(daynr,HOUR,LATI,LONGI,hppo,SUC,zxz,sap,sup)
        TCOR2 = TCOR2CAL_2020(hppo,hour,modip,pf107,sap,sup)
        zneppo = log(XE_1_2020(hppo)/nmf2s)
        palogne(5)=palogne(5)-zneppo
        CALL SOCO_2020(daynr,HOUR,LATI,LONGI,hpt,SUC,zxz,sap,sup)
        TCOR2 = TCOR2CAL_2020(hpt,hour,modip,pf107,sap,sup)
        znept = log(XE_1_2020(hpt)/nmf2s)
...
c
        if(jf(4)) then
          B0=B0_98_2020(HOUR,SAX200,SUX200,NSEASN,RSSN,LONGI,MODIP)
	  B1=HPOL_2020(HOUR,1.9,2.6,SAX200,SUX200,1.,1.)
        else if (jf(31)) then
 	  CALL SHAMDB0D_2020 (RLAT,FLON,ZMONTH,RSSN,B0)
          CALL SHAB1D_2020 (LATI,FLON,ZMONTH,RSSN,B1)
        else
          CALL ROGUL_2020(SEADAY,XHI3,SEAX,GRAT)
...
c
        IF(FOF1IN) THEN
          ...
          goto 2918
        ELSE
          FOF1=FOF1ED_2020(ABSMBR,RSSN,XHI3)
...
c
        if(f1_ocpro) then
          call f1_prob_2020(xhi3,mlat,rssn,f1pbw,f1pbl)
          f1pb = f1pbw
...
c
      ...
      HDEEP=HPOL_2020(HOUR,10.5/DELA,28.,SAX110,SUX110,1.,1.)
      WIDTH=HPOL_2020(HOUR,17.8/DELA,45.+22./DELA,SAX110,SUX110,1.,1.)
      DEPTH=HPOL_2020(HOUR,XDEL,81.,SAX110,SUX110,1.,1.)
      DLNDH=HPOL_2020(HOUR,DNDHBR,.06,SAX110,SUX110,1.,1.)
      IF(DEPTH.LT.1.0) GOTO 600
        IF(ENIGHT) DEPTH=-DEPTH
        CALL TAL_2020(HDEEP,DEPTH,WIDTH,DLNDH,EXT,E)
        IF(.NOT.EXT) GOTO 667
...
2727  continue
      hmex=hme-9.
      NMD=XMDED_2020(XHI1,RSSN,4.0E8)
      HMD=HPOL_2020(HOUR,81.0,88.0,SAX80,SUX80,1.,1.)
      F(1)=HPOL_2020(HOUR,0.02+0.03/DELL,0.05,SAX80,SUX80,1.,1.)
      F(2)=HPOL_2020(HOUR,4.6,4.5,SAX80,SUX80,1.,1.)
      F(3)=HPOL_2020(HOUR,-11.5,-4.0,SAX80,SUX80,1.,1.)
      FP1=F(1)
      FP2=-FP1*FP1/2.0
...
c
      if(.not.dreg) then
          ...
          f5sw=0.
          f6wa=0.
          call DRegion_2020(xhi1,month,f107d,vKp,f5SW,f6WA,elg)
          do ii=1,11
            ddens(1,ii)=-1.
...
c
       XXE1_2020=XE2_2020(HEF)
       if(nmf1.lt.xxe1_2020) goto 9427

       CALL
     &REGFA1_2020(HEF,HMF2,XXE1_2020,NMF2S,0.001,NMF1,XE2_2020,SCHALT,HM
     &F1)
...
3763    if(xf1.lt.nmes) then
        ...
        CALL
     &REGFA1_2020(hf1,HF2,XF1,XF2,0.001,NMES,XE3_1_2020,SCHALT,HST)
        if(schalt) goto 3885
        HZ=(HST+HF1)/2.0
        GOTO 4933
...
6153    IF(HMF1IN) THEN
        ...
        HHMF2 = HMF2
        CALL
     &INILAY_2020(FNIGHT,F1REG,NMF2S,NMF1,NMES,VNER,HHMF2,HMF1M,HME,
...
4933  HTA=60.0
      ...
      SEC=hourut*3600.
      CALL APFMSIS_2020(ISDATE,HOURUT,IAPO)
      ...
      CALL TSELEC_2020(SWMI)
      CALL GTD7_2020(IYD,SEC,HEQUI,LATI,LONGI,HOUR,F10781OBS,
...
c Te-MAXIMUM based on JICAMARCA and ARECIBO data
        ...
        HMAXN=150.
        AHH(2)=HPOL_2020(HOUR,HMAXD,HMAXN,SAX200,SUX200,1.,1.)
        TMAXD=800.*EXP(-(MLAT/33.)**2)+1500.
        secni=(24.-longi/15)*3600.
        CALL GTD7_2020(IYD,SECNI,HMAXN,LATI,LONGI,0.0,F10781OBS,
     &        F107YOBS,IAPO,0,D_MSIS,T_MSIS)
        TMAXN=T_MSIS(2)
        ATE(2)=HPOL_2020(HOUR,TMAXD,TMAXN,SAX200,SUX200,1.,1.)
...
C Te(3000km) from ISIS-1 (Brace and Theis, 1981)
              DIPLAT=MAGBR
              CALL TEBA_2020(DIPLAT,HOUR,NSEASN,TEA)
              AHH(3)=300.
              AHH(4)=400.
              AHH(5)=600.
              AHH(6)=1400.
...
c Te(600km) from AEROS, Spenner and Plugge (1979)
              ...
              ATE(5)=HPOL_2020(HOUR,TET,TEN,SAX300,SUX300,1.5,1.5)
...
c isa for solar activity correction: isa=0 sol activity corr off
              isa=0
              if(jf(42)) isa=1
              call elteik_2020(isa,invdip_old,xmlt,daynr,
...
c Option to use Te = f(Ne) relation at ahh(3), ahh(4)
          ...
          IF(TENEOP) THEN
              DO 3395 I=1,2
3395              IF(TECON(I)) ATE(I+2)=TEDE_2020(AHH(I+2),XNAR(I),-COV)
              endif

c Te corrected and Te > Tn enforced

      CALL GTD7_2020(IYD,SEC,AHH(2),LATI,LONGI,HOUR,F10781OBS,
     &        F107YOBS,IAPO,0,D_MSIS,T_MSIS)
      TNAHH2=T_MSIS(2)
      IF(ATE(2).LT.TNAHH2) ATE(2)=TNAHH2
      STTE1=(ATE(2)-ATE(1))/(AHH(2)-AHH(1))
      DO 1901 I=2,6
         CALL GTD7_2020(IYD,SEC,AHH(I+1),LATI,LONGI,HOUR,F10781OBS,
     &        F107YOBS,IAPO,0,D_MSIS,T_MSIS)
         TNAHHI=T_MSIS(2)
...
c Starting height is 200km. Below 200km Ti=Tn
      	HS=200.
      	XSM(1)=HS
      	CALL GTD7_2020(IYD,SEC,HS,LATI,LONGI,HOUR,F10781OBS,F107YOBS,
...
c JSOL=0 solar activity dependence excluded
	...
	xsm(4)=600
	xsm(5)=850
      	CALL IONTIF_2020(JSOL,INVDIP_OLD,XMLT,DAYNR,PF107OBS,TIV,SIGTV)

c Tn < Ti < Te enforced at xsm(2:5)
        do 2391 i10=2,5
          HTIX=XSM(i10)
          TEXSM=BOOKER1_2020(HTIX,5,ATE1,AHH,STTE,DTE)
      	  CALL GTD7_2020(IYD,SECNI,XSM(i10),LATI,LONGI,0.0,
...
2397    X=X+XTTS
        ...
        TEX=BOOKER1_2020(X,5,ATE1,AHH,STTE,DTE)
        TIX=BOOKER1_2020(X,MXSM,TNHS,XSM,MM,DTI)
        ...
        if(xteti.gt.xsm(5)+100) then
           xsm(6)=xteti
           TEX5=BOOKER1_2020(XTETI,5,ATE1,AHH,STTE,DTE)
...
c Ti(430km) for specified time using HPOL
      	TI1=TIN1
      	IF(TID1.GT.TIN1) TI1=HPOL_2020(HOUR,TID1,TIN1,SAX300,SUX300,1.,
     &1.)

c Tn < Ti < Te enforced at 430 km
        TEN1=BOOKER1_2020(XSM1,5,ATE1,AHH,STTE,DTE)
      	CALL GTD7_2020(IYD,SECNI,XSM1,LATI,LONGI,0.0,F10781OBS,
     &        F107YOBS,IAPO,0,D_MSIS,T_MSIS)
      	TNN1=T_MSIS(2)
...
c Gradient for second segment
      	MM(2)=HPOL_2020(HOUR,3.0,0.0,SAX300,SUX300,1.,1.)
      	XSM(3)=HTE
...
300   CALL SOCO_2020(daynr,HOUR,LATI,LONGI,height,SUNDEC,XHI,SAX,SUX)
...
2919    format(/'At heights above 2000 km a first-order ',
     &    'plasmaspheric extension is applied up to ',I5,'km.')
      ...
      IF(LAYVER) THEN
          ELEDE=-9.
          IF(IIQU.LT.2) ELEDE=
     &            XEN_2020(HEIGHT,HMF2,NMF2S,HME,4,HXL,SCL,AMP)
          outf(1,kk)=elede
          goto 330
          endif
...
c
      if(.not.dreg.and.height.le.140.) then
            elede=-1.
            call F00_2020(HEIGHT,LATI,DAYNR,XHI,F107D,EDENS,IERROR)
...
330   IF(NOTEM.and.(NOION.or..not.RBTT)) GOTO 7108
      ...
      CALL GTD7_2020(IYD,SEC,HEIGHT,LATI,LONGI,HOUR,F10781OBS,
     &        F107YOBS,IAPO,0,D_MSIS,T_MSIS)
      TNH=T_MSIS(2)
      TEH=TNH
      if(HEIGHT.GT.HEQUI) TEH=BOOKER1_2020(HEIGHT,5,ATE1,AHH,STTE,DTE)
      if(HEIGHT.LE.HS) THEN
         TIH=TNH
      ELSE IF(HEIGHT.LT.XTETI) THEN
         TIH=BOOKER1_2020(HEIGHT,MXSM,TNHS,XSM,MM,DTI)
      ELSE
         TIH=TEH
      ENDIF
c
...
c Triskova-Truhlik-Smilauer-2003 model
          call igrf_sub_2020(lati,longi,ryear,height,fl,icode,dipl,babs)
          if(fl.gt.10.) fl=10.
          invdip=INVDPC_2020(FL,DIMO,BABS,DIPL)
          call CALION_2020(invdip,xmlt,height,daynr,pf107obs,
...
c Richards-Bilitza-Voglozin-2010 IDC model
     ...
     &      f107yobs,IAPO,48,D_MSIS,T_MSIS)
          ...
          Den_NO = 0.0
          rn = 0.0
          CALL CHEMION_2020(jprint,height,F107YOBS,F10781OBS,TEH,TIH,
...
c Danilov-Smirnova-1995 model below 300km and Danilov-Yaichnikov-1985 model above
        call iondani_2020(iday,iseamon,height,xhi,lati,f107365,dion)
        ROX=DION(1)
        RHX=DION(2)
        RNX=DION(3)
        RHEX=DION(4)
...
c
      ...
      if(.not.dreg) then
        do ii=1,11
          Htemp=55+ii*5
          outf(14,ii)=-1.
          if(Htemp.ge.65.) outf(14,ii)=XE6_2020(Htemp)
          outf(14,11+ii)=-1.
          call F00_2020(Htemp,LATI,DAYNR,XHI1,F107D,EDENS,IERROR)
...
c
        CALL vfjmodelrocstart_2020(FJM)
        CALL vfjmodelrocinit_2020(F107D,DAYNR,JSEA,JF107)
        CALL vfjmodelroc_2020(FJM,HOUR,LONGI,JSEA,JF107,DRIFT)
        endif
...
c
      ...
      if(abs(lati).gt.25.0) goto 1937
        ...
        call spreadf_brazil_2020(daynr,idayy,f107d,lati,osfbr)
...
c
c
        subroutine iri_web_2020(
     &jmag,jf,alati,along,iyyyy,mmdd,iut,dhour,
...
1249            oarr(i)=oar(i)
            xhour=dhour+iut*25.
            call IRI_SUB_2020(JF,JMAG,ALATI,ALONG,IYYYY,MMDD,
     &                  XHOUR,VBEG,VEND,VSTP,a,OARR)
            if(h_tec_max.gt.50.) then
c                call iri_tec (50.,h_tec_max,2,tec,tect,tecb)
                call IRITEC_2020(ALATI,ALONG,jmag,jf,iyyyy,mmdd,
