...
C
C
      SUBROUTINE FIELDG_2020(DLAT,DLONG,ALT,X,Y,Z,F,DIP,DEC,SMODIP)
...
C SHEIK,1977.
      DIMENSION H(144),XI(3),G_2020(144),FEL1(72),FEL2(72)
      COMMON/CONST/UMR,PI
...
300   IL=IH-I
      ...
      DO 600 M=3,I,2
      H(IL+M+1)=G_2020(IL+M+1)+Z1*H(IH+M+1)+X1*(H(IH+M+3)-H(IH+M-1))-
     &Y1*(H(IH+M+2)+H(IH+M-2))
      H(IL+M)=G_2020(IL+M)+Z1*H(IH+M)+X1*(H(IH+M+2)-H(IH+M-2))+
     &Y1*(H(IH+M+3)+H(IH+M-1))
600   CONTINUE
500   H(IL+2)=G_2020(IL+2)+Z1*H(IH+2)+X1*H(IH+4)-Y1*(H(IH+3)+H(IH))
      H(IL+1)=G_2020(IL+1)+Z1*H(IH+1)+Y1*H(IH+4)+X1*(H(IH+3)-H(IH))
400   H(IL)=G_2020(IL)+Z1*H(IH)+2.0*(X1*H(IH+1)+Y1*H(IH+2))
700   IH=IL
      IF(I.GE.K) GOTO 300
200   CONTINUE
...
c
c
        subroutine igrf_sub_2020(xlat,xlong,year,height,
     &          xl,icode,dipl,babs)
c-----------------------------------------------------------------------
c INPUT:
...
c      CALL FELDCOF(YEAR,DIMO)
      CALL FELDG_2020(LATI,LONGI,HEIGHT,BNORTH,BEAST,BDOWN,BABS)
      CALL SHELLG_2020(LATI,LONGI,HEIGHT,XL,ICODE,BAB1)
      DIPL=ATAN(BDOWN/2.0/sqrt(BNORTH*BNORTH+BEAST*BEAST))/umr
      RETURN
      END
c
c
      subroutine igrf_dip_2020(
     &xlat,xlong,year,height,dec,dip,dipl,ymodip)
c-----------------------------------------------------------------------
c INPUT:
c    xlat      geodatic latitude in degrees
c    xlong     geodatic longitude in degrees
...
c-----------------------------------------------------------------------
	  ...
	  h = height
c      CALL FELDCOF(YEAR,DIMO)
      CALL FELDG_2020(XLATI,XLONGI,H,BNORTH,BEAST,BDOWN,BABS)
...
C
C
      SUBROUTINE SHELLG_2020(GLAT,GLON,ALT,FL,ICODE,B0)
...
c-----------------------------------------------------------------------
      DIMENSION         V(3),U(3,3),P(8,33335),SP(3)
...
5     P(1,N+1)=P(1,N)+STEP12*(23.*P(4,N)-16.*P(4,N-1)+5.*P(4,N-2))
      ...
      P(3,N+1)=P(3,N)+STEP
      CALL STOER_2020(P(1,N+1),BQ3,R3)
C*****SEARCH FOR LOWEST MAGNETIC FIELD STRENGTH
      IF(BQ3.LT.BEQU) THEN
        IEQU=N+1
        BEQU=BQ3
...
C
C
      SUBROUTINE FELDG_2020(GLAT,GLON,ALT,BNORTH,BEAST,BDOWN,BABS)
c-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      CHARACTER*13      NAME
      COMMON/IGRF2/XI(3),H(196)
      COMMON/MODEL/NMAX,TIME,G_2020(196),NAME
...
2     	DO 3 M=3,I,2
      		H(IL+M+1)=G_2020(IL+M+1)+Z*H(IH+M+1)+X*(H(IH+M+3)-
     A                H(IH+M-1))-Y*(H(IH+M+2)+H(IH+M-2))
3     		H(IL+M)=G_2020(IL+M)+Z*H(IH+M)+X*(H(IH+M+2)-
     A                H(IH+M-2))+Y*(H(IH+M+3)+H(IH+M-1))

4     	H(IL+2)=G_2020(IL+2)+Z*H(IH+2)+X*H(IH+4)-Y*(H(IH+3)+H(IH))
      	H(IL+1)=G_2020(IL+1)+Z*H(IH+1)+Y*H(IH+4)+X*(H(IH+3)-H(IH))
5     	H(IL)=G_2020(IL)+Z*H(IH)+2.*(X*H(IH+1)+Y*H(IH+2))
      	IH=IL
      	IF(I.GE.K) GOTO 1
6     	CONTINUE
...
C-- GET IGRF COEFFICIENTS FOR THE BOUNDARY YEARS
        CALL GETSHC_2020 (IU, FIL1, NMAX1, ERAD, GH1, IER)
            IF (IER .NE. 0) STOP
        CALL GETSHC_2020 (IU, FIL2, NMAX2, ERAD, GH2, IER)
            IF (IER .NE. 0) STOP
C-- DETERMINE IGRF COEFFICIENTS FOR YEAR
        IF (L .LE. NUMYE-1) THEN
          CALL INTERSHC_2020 (YEAR, DTE1, NMAX1, GH1, DTE2,
     1          NMAX2, GH2, NMAX, GHA)
        ELSE
          CALL EXTRASHC_2020 (YEAR, DTE1, NMAX1, GH1, NMAX2,
     1          GH2, NMAX, GHA)
        ENDIF
...
C
C
        SUBROUTINE GETSHC_2020 (IU, FSPEC, NMAX, ERAD, GH, IER)
C ===============================================================
...
C
C
        SUBROUTINE INTERSHC_2020 (DATE, DTE1, NMAX1, GH1, DTE2,
     1                        NMAX2, GH2, NMAX, GH)
...
C
C
        SUBROUTINE EXTRASHC_2020 (DATE, DTE1, NMAX1, GH1, NMAX2,
     1                        GH2, NMAX, GH)
C ===============================================================               C
C       Version 1.01                                                   C
...
C
C
      SUBROUTINE GEODIP_2020(IYR,SLA,SLO,DLA,DLO,J)
C ===============================================================
...
C        RH = (RE + HI)/RE
         ...
         COL = (90.- SLA)*UMR
         RLO = SLO*UMR
      CALL SPHCAR_2020(R,COL,RLO,X,Y,Z,1)
      CALL GEOMAG_2020(X,Y,Z,XM,YM,ZM,1,IYR)
      CALL SPHCAR_2020(RM,TH,PF,XM,YM,ZM,-1)
         SZM = ZM
         DLO = PF/UMR
         DCO = TH/UMR
         DLA = 90.- DCO
...
1234     continue
      COL = (90.- DLA)*UMR
      RLO = DLO*UMR
      CALL SPHCAR_2020(R,COL,RLO,XM,YM,ZM,1)
      CALL GEOMAG_2020(X,Y,Z,XM,YM,ZM,-1,IYR)
      CALL SPHCAR_2020(RM,TH,PF,X,Y,Z,-1)
        SZM = ZM
        SLO = PF/UMR
        SCO = TH/UMR
        SLA = 90.- SCO
...
C
C
		function fmodip_2020(xlat)

		common/findRLAT/xlong,year

      	call igrf_dip_2020(xlat,xlong,year,300.,dec,dip,dipl,ymodip)
      	fmodip_2020=ymodip
...
C
C
      SUBROUTINE GEOCGM01_2020(ICOR,IYEAR,HI,DAT,PLA,PLO)
C  *********************************************************************
...
C  middle latitudes
      ...
      IF (ICOR.EQ. 1) THEN
         ...
         CALL GEOCOR_2020(SLAR,SLOR,RH,DLA,DLO,CLAR,CLOR,PMR)
         DAT(3,1) = CLAR
         DAT(4,1) = CLOR
...
C  middle latitudes
         ...
         CALL CORGEO_2020(SLAR,SLOR,RH,DLA,DLO,CLAR,CLOR,PMR)
         DAT(1,1) = SLAR
         DAT(2,1) = SLOR
...
C  reference in GEOLOW)
           ...
           CALL GEOLOW_2020(SLAR,SLOR,RH,CLAR,CLOR,RBM,SLAC,SLOC)
           DAT(3,1) = CLAR
           DAT(4,1) = CLOR
...
C  Conjugate point coordinates at low latitudes
           ...
           DAT(1,2) = SLAC
           DAT(2,2) = SLOC
           CALL GEOCOR_2020(SLAC,SLOC,RH,DAA,DOO,CLAC,CLOC,RBM)
           IF(CLAC.GT.999.)
     +       CALL GEOLOW_2020(SLAC,SLOC,RH,CLAC,CLOC,RBM,SLAL,SLOL)
           DAT(3,2) = CLAC
           DAT(4,2) = CLOC
           DAT(5,2) = RBM
...
C  middle latitudes
           ...
           DAT(3,2) = CLAC
           DAT(4,2) = CLOC
           CALL CORGEO_2020(SLAC,SLOC,RH,DAA,DOO,CLAC,CLOC,PMC)
           DAT(1,2) = SLAC
           DAT(2,2) = SLOC
...
C  Earth's surface for the starting point
      ...
      IF(RH.GT.1..and.CLAR.LT.999..and.CLAR.LT.999.) THEN
        CALL
     &FTPRNT_2020(RH,SLAR,SLOR,CLAR,CLOR,ACLAR,ACLOR,SLARF,SLORF,1.)
            DAT(1,3) = SLARF
            DAT(2,3) = SLORF
            DAT(3,3) = ACLAR
            DAT(4,3) = ACLOR
C  and for the conjugate point
        CALL
     &FTPRNT_2020(RH,SLAC,SLOC,CLAC,CLOC,ACLAC,ACLOC,SLACF,SLOCF,1.)
            DAT(1,4) = SLACF
            DAT(2,4) = SLOCF
            DAT(3,4) = ACLAC
            DAT(4,4) = ACLOC
...
C  poles for a given year at the altitude RH and Earth's surface (1-Re)
        ...
        CALL CORGEO_2020(PLAN,PLON,RH,DAA,DOO, 90.,360.,PMP)
            PLAN1 = PLAN
            PLON1 = PLON

        CALL CORGEO_2020(PLAS,PLOS,RH,DAA,DOO,-90.,360.,PMP)
            PLAS1 = PLAS
            PLOS1 = PLOS

        IF(RH.GT.1.) THEN
          CALL CORGEO_2020(PLAN1,PLON1,1.,DAA,DOO, 90.,360.,PMP)
          CALL CORGEO_2020(PLAS1,PLOS1,1.,DAA,DOO,-90.,360.,PMM)
        ENDIF
...
C  Computation of the IGRF components
        CALL MFC_2020(SLAJ,SLOJ,RJ,BTR,BFR,BRR)
          DAT(6,j) = BTR
          DAT(7,j) = BFR
          DAT(8,j) = BRR
...
C  and high latitudes where CGM --> GEO is permitted.
        ...
        OVL = OVL_ANG_2020(SLAJ,SLOJ,CLAJ,CLOJ,RJ)
          DAT(9,j) = OVL
...
C  the North (South) CGM pole
        ...
        AZM = AZM_ANG_2020(SLAJ,SLOJ,CLAJ,PLAJ,PLOJ)
          DAT(10,j) = AZM

C  Computation of the MLT midnight (in UT)
        CALL MLTUT_2020(SLAJ,SLOJ,CLAJ,PLAJ,PLOJ,UT)
          DAT(11,j) = UT
...
C
C
      real function OVL_ANG_2020(sla,slo,cla,clo,rr)
C  *********************************************************************
...
C  *********************************************************************
      ...
      real cgmgla_2020,cgmglo_2020,dfridr_2020
      logical cr360,cr0

      external cgmgla_2020,cgmglo_2020,dfridr_2020
...
C  GEOLON if |GEOLAT| = 90 deg. - see CGMGLO for details.
           ...
           hom = dfridr_2020(cgmgla,clo,step,err1)

         denom = dfridr_2020(cgmglo_2020,clo,step,err2)

         denom = denom*cos(sla*0.017453293)

       ovl_ang_2020 = -atan2(hom,denom)

       ovl_ang_2020 = OVL_ANG*57.2957751

      return
      end
C
C
      real function cgmgla_2020(clon)
C  *********************************************************************
...
C  *********************************************************************
      ...
      common/cgmgeo/cclat,cr360,cr0,rh
       ...
       call CORGEO_2020(geolat,geolon,rr,dla,dlo,cclat,clon,pmi)
         cgmgla = geolat
...
C
C
      real function cgmglo_2020(clon)
C *********************************************************************
...
C
C
      FUNCTION DFRIDR_2020(func,x,h,err)
C **********************************************************************
...
C **********************************************************************
      ...
      INTEGER NTAB
      REAL dfridr_2020,err,h,x,func,CON,CON2,BIG,SAFE
      LOGICAL mess
...
100       FORMAT('h must be nonzero in dfridr_2020')
          return
          endif
       hh = h
...
C
C
      real function AZM_ANG_2020(sla,slo,cla,pla,plo)
C  *********************************************************************
     ...
     +  f6.2,' are not in the same hemisphere: AZM_ANG is incorrect!')
      ...
      RAD = 0.017453293
        ...
        if(sla.ge.0.) then
         ...
         alfa = atan2(sb,st)
         azm_ang_2020 = alfa/rad

      RETURN
      END
C
C
      SUBROUTINE MLTUT_2020(SLA,SLO,CLA,PLA,PLO,UT)
C  *********************************************************************
...
C
C
        SUBROUTINE MFC_2020(SLA,SLO,R,H,D,Z)
C  *********************************************************************
...
C  Computation of all geomagnetic field components
        ...
        RLO = SLO*0.017453293
       CALL IGRF_2020(IYR,NM,R,RLA,RLO,BR,BT,BF)
          X = -BT
          Y =  BF
          Z = -BR
          H = SQRT(X**2+Y**2)
...
C
C
      SUBROUTINE FTPRNT_2020(RH,SLA,SLO,CLA,CLO,ACLA,ACLO,SLAF,SLOF,RF)
C  *********************************************************************
...
C  Defining the Altitude Adjusted CGM coordinates for a given point
        ...
        CALL CORGEO_2020(SLAF,SLOF,RF,DLAF,DLOF,ACLA,ACLO,PMIF)
...
C  Start from an initial point
        ...
        RSLO = SLO*0.0174533
        CALL SPHCAR_2020(R,RSLA,RSLO,XF,YF,ZF,1)
         RF1 = R
         XF1 = XF
         YF1 = YF
         ZF1 = ZF

  255   CALL SHAG_2020(XF,YF,ZF,DS)
         ...
         IF (RR.GT.RF) THEN
           ...
           DR10 = DR1 + DR0
              IF(DR10.NE.0.) THEN
                 DS = DS*(DR1/DR10)
                 CALL SHAG_2020(XF1,YF1,ZF1,DS)
              ENDIF
           CALL SPHCAR_2020(RR,SLAF,SLOF,XF1,YF1,ZF1,-1)
...
C
C
      SUBROUTINE GEOLOW_2020(SLAR,SLOR,RH,CLAR,CLOR,RBM,SLAC,SLOC)
C  *********************************************************************
...
C  in the Northern Hemisphere
             ...
             DO JC = 61,91
               SLA = 90.-(JC-1)
               CALL GEOCOR_2020(SLA,SLO,RH,DAA,DOO,CLA,CLO,PMM)
               IF(CLA.GT.999.) THEN
                 NDIR=1
                 GOTO 53
               ENDIF
...
C  Here CLA is used only to calculate FRAC
           ...
           SLL = SLO*0.017453293
           CALL IGRF_2020(IYR,NM,RM,SLM,SLL,BR,BT,BF)
           SZ = -BR
           CALL SPHCAR_2020(RM,SLM,SLL,XGEO,YGEO,ZGEO,1)
...
C  Keep two consequently computed points to define B-min
           ...
           DO 7 I = 1,2
              DD = DS
              CALL SHAG_2020(XGEO,YGEO,ZGEO,DD)
   11         IF(I.NE.1) GOTO 9
              XBM1 = XGEO
              YBM1 = YGEO
              ZBM1 = ZGEO
...
C  magnetic field line (could happen at very near geomagnetic equator)
   ...
   21      CONTINUE
           ...
           CALL SPHCAR_2020(RBM1,RLA,RLO,XBM1,YBM1,ZBM1,-1)
...
C  Computation of the magnetically conjugate point at low latitudes
   ...
   54      continue
           if(ihem.eq.3) then
              ...
              DS = DSD
   55         continue
              CALL SHAG_2020(XBM1,YBM1,ZBM1,DS)
              ...
              ELSE
                 ...
                 IF(DR10.NE.0.) THEN
                    DS = DS*(DR1/DR10)
                    RM = R1
                    CALL SHAG_2020(X1,Y1,Z1,DS)
                    ENDIF
                 CALL SPHCAR_2020(RR,SLAC,SLOC,X1,Y1,Z1,-1)
...
C
C
      SUBROUTINE CORGEO_2020(SLA,SLO,RH,DLA,DLO,CLA,CLO,PMI)
C  *********************************************************************
...
C  RFI = R if COL = 90 deg.
    ...
    3 CALL SPHCAR_2020(R0,SCLA,RLO,XM,YM,ZM,1)
      CALL GEOMAG_2020(X,Y,Z,XM,YM,ZM,-1,IYR)
         RL = R0
       FRAC = -0.03/(1. + 3./(RL - 0.6))
...
C  Define intersection with the start surface
    ...
    7   DR1 = ABS(RH - R1)
      ...
      CALL SPHCAR_2020(R,GTET,GXLA,X1,Y1,Z1,-1)
        GTH = GTET*57.2957751
        SLO = GXLA*57.2957751
        SLA = 90. - GTH
      CALL GEOMAG_2020(X1,Y1,Z1,XM,YM,ZM,1,IYR)
      CALL SPHCAR_2020(RM,TH,PF,XM,YM,ZM,-1)
        DLO = PF*57.2957751
...
C  30 degrees (see the paper referenced in GEOLOW)
      ...
      IF(ABS(SLA).LT.30..OR.ABS(CLA).LT.30.) THEN
          CALL GEOCOR_2020(SLA,SLO,RH,DLS,DLS,CLAS,CLOS,PMS)

      IF(CLAS.GT.999.)
     &CALL GEOLOW_2020(SLA,SLO,RH,CLAS,CLOS,RBM,SLAC,SLOC)
...
C
C
      SUBROUTINE GEOCOR_2020(SLA,SLO,RH,DLA,DLO,CLA,CLO,PMI)
C  *********************************************************************
...
C  Trace the IGRF magnetic field line to the dipole equatorial plane
   ...
   3     NM = (1. + 9./R) + 0.5
      ...
      CALL SHAG_2020(X,Y,Z,DS)
      CALL GEOMAG_2020(X,Y,Z,XM,YM,ZM,1,IYR)
      CALL SPHCAR_2020(R,C,S,XM,YM,ZM,-1)
...
C  dipole equatorial plane, no CGM coordinates can be calculated
   ...
   9  CALL GEOMAG_2020(X,Y,Z,XM,YM,ZM,1,IYR)
      CALL SPHCAR_2020(R,GTET,GXLA,XM,YM,ZM,-1)
         ST = ABS(SIN(GTET))
...
C  *********************************************************************
      ...
      COMMON/A5/DS3
          ...
          DS3 = -DS/3.
      CALL RIGHT_2020(X,Y,Z,R11,R12,R13)
      CALL RIGHT_2020(X+R11,Y+R12,Z+R13,R21,R22,R23)
      CALL RIGHT_2020(X+.5*(R11+R21),Y+.5*(R12+R22),Z+.5*(R13+R23),
     *R31,R32,R33)
      CALL RIGHT_2020(X+.375*(R11+3.*R31),Y+.375*(R12+3.*R32),
     *Z+.375*(R13+3.*R33),R41,R42,R43)
      CALL RIGHT_2020(X+1.5*(R11-3.*R31+4.*R41),
...
C
C
      SUBROUTINE RIGHT_2020(X,Y,Z,R1,R2,R3)
C  *********************************************************************
...
C  *********************************************************************
      ...
      CALL SPHCAR_2020(R,T,F,X,Y,Z,-1)
      CALL IGRF_2020(IYR,NM,R,T,F,BR,BT,BF)
      CALL BSPCAR_2020(T,F,BR,BT,BF,BX,BY,BZ)
...
C
C
	SUBROUTINE IGRF_2020(IY,NM,R,T,F,BR,BT,BF)
C  *********************************************************************
...
C  *********************************************************************
      ...
      DIMENSION A(11),B(11),DG(45),DH(45),G_2020(66),H(66),REC(66),
...
1900  F2=(IYR-1900)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1900(N)*F1+G1905(N)*F2
...
1905  F2=(IYR-1905)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1905(N)*F1+G1910(N)*F2
...
1910  F2=(IYR-1910)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1910(N)*F1+G1915(N)*F2
...
1915  F2=(IYR-1915)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1915(N)*F1+G1920(N)*F2
...
1920  F2=(IYR-1920)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1920(N)*F1+G1925(N)*F2
...
1925  F2=(IYR-1925)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1925(N)*F1+G1930(N)*F2
...
1930  F2=(IYR-1930)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1930(N)*F1+G1935(N)*F2
...
1935  F2=(IYR-1935)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1935(N)*F1+G1940(N)*F2
...
1940  F2=(IYR-1940)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1940(N)*F1+G1945(N)*F2
...
1945  F2=(IYR-1945)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1945(N)*F1+G1950(N)*F2
...
1950  F2=(IYR-1950)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1950(N)*F1+G1955(N)*F2
...
1955  F2=(IYR-1955)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1955(N)*F1+G1960(N)*F2
...
1960  F2=(IYR-1960)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1960(N)*F1+G1965(N)*F2
...
1965  F2=(IYR-1965)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1965(N)*F1+G1970(N)*F2
...
1970  F2=(IYR-1970)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1970(N)*F1+G1975(N)*F2
...
1975  F2=(IYR-1975)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1975(N)*F1+G1980(N)*F2
...
1980  F2=(IYR-1980)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1980(N)*F1+G1985(N)*F2
...
1985  F2=(IYR-1985)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1985(N)*F1+G1990(N)*F2
...
1990  F2=(IYR-1990)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1990(N)*F1+G1995(N)*F2
...
1995  F2=(IYR-1995)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G1995(N)*F1+G2000(N)*F2
...
2000  F2=(IYR-2000)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G2000(N)*F1+G2005(N)*F2
...
2005  F2=(IYR-2005)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G2005(N)*F1+G2010(N)*F2
...
2010  F2=(IYR-2010)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G2010(N)*F1+G2015(N)*F2
...
2015  F2=(IYR-2015)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G2015(N)*F1+G2020(N)*F2
...
2020  F2=(IYR-2020)/5.
      F1=1.-F2
      DO N=1,66
         G_2020(N)=G2020(N)*F1+G2025(N)*F2
...
C
      ...
      SUBROUTINE RECALC_2020(IYR,IDAY,IHOUR,MIN,ISEC)
C  *********************************************************************
...
C  IS TO BE DONE  (IHOUR>24 IS THE AGREED CONDITION FOR THIS CASE):
   ...
   5   IF (IHOUR.GT.24) RETURN
      ...
      CALL SUN_2020(IY,IDAY,IHOUR,MIN,ISEC,GST,SLONG,SRASN,SDEC)
...
C
C
      SUBROUTINE SPHCAR_2020(R,TETA,PHI,X,Y,Z,J)
C ===============================================================
...
C
C
      SUBROUTINE BSPCAR_2020(TETA,PHI,BR,BTET,BPHI,BX,BY,BZ)
C  *********************************************************************
...
C
C
      SUBROUTINE GEOMAG_2020(XGEO,YGEO,ZGEO,XMAG,YMAG,ZMAG,J,IYR)
C ===============================================================
...
C
C
      SUBROUTINE MAGSM_2020(XMAG,YMAG,ZMAG,XSM,YSM,ZSM,J)
C  *********************************************************************
...
C
C
       SUBROUTINE SMGSM_2020(XSM,YSM,ZSM,XGSM,YGSM,ZGSM,J)
C  *********************************************************************
...
C
C
       SUBROUTINE CLCMLT_2020(IYYYY,DDD,UTHR,GLAT,GLON,MLT)
C--------------------------------------------------------------------
...
C--------------------------------------------------------------------
       ...
       ZG=SIN(GLAT*DTOR)
       CALL DPMTRX_2020(IYYYY,DDD,XXM,YYM,ZZM)
...
C
       ...
       CALL SUN_2020 (IYYYY,DDD+1,IHOUR,MIN,ISEC,GST,SLONG,SRASN,SDEC)
       BE=GST
       CAL=COS(SRASN)
       SA(3)=SIN(SDEC)
       SA(1)=COS(SDEC)
...
C
C
       SUBROUTINE DPMTRX_2020(IYYYY,DDD,XM,YM,ZM)
C--------------------------------------------------------------------------
...
C--------------------------------------------------------------------------
       ...
       REAL interp_2020,YEAR,M,MXI,MYI,MZI,ZM12
