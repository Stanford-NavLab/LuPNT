...
C        Test for changed input
      V1=VTST7_2020(IYD,SEC,GLAT,GLONG,STL,F107A,F107,AP,1)
C       Latitude variation of gravity (none for SW(2)=0)
      XLAT=GLAT
      IF(SW(2).EQ.0) XLAT=45.
      CALL GLATF_2020(XLAT,GSURF,RE)
C
      XMM=PDM(5,3)
C
...
C         or altitude above ZN2(1) in mesosphere
     ...
     & THEN
        ...
        CALL
     &GTS7_2020(IYD,SEC,ALTT,GLAT,GLONG,STL,F107A,F107,AP,MSS,DS,TS)
        DM28M=DM28
C         metric adjustment
...
C         Only calculate nodes if input changed
       IF(V1.EQ.1..OR.ALAST.GE.ZN2(1)) THEN
        TGN2(1)=TGN1(2)
        TN2(1)=TN1(5)
        TN2(2)=PMA(1,1)*PAVGM(1)/(1.-SW(20)*GLOB7S_2020(PMA(1,1)))
        TN2(3)=PMA(1,2)*PAVGM(2)/(1.-SW(20)*GLOB7S_2020(PMA(1,2)))
        TN2(4)=PMA(1,3)*PAVGM(3)/(1.-
     &SW(20)*SW(22)*GLOB7S_2020(PMA(1,3)))
        TGN2(2)=PAVGM(9)*PMA(1,10)*(1.+
     &SW(20)*SW(22)*GLOB7S_2020(PMA(1,10)))
...
C         Only calculate nodes if input changed
        IF(V1.EQ.1..OR.ALAST.GE.ZN3(1)) THEN
         TGN3(1)=TGN2(2)
         TN3(2)=PMA(1,4)*PAVGM(4)/(1.-SW(22)*GLOB7S_2020(PMA(1,4)))
         TN3(3)=PMA(1,5)*PAVGM(5)/(1.-SW(22)*GLOB7S_2020(PMA(1,5)))
         TN3(4)=PMA(1,6)*PAVGM(6)/(1.-SW(22)*GLOB7S_2020(PMA(1,6)))
         TN3(5)=PMA(1,7)*PAVGM(7)/(1.-SW(22)*GLOB7S_2020(PMA(1,7)))
         TGN3(2)=PMA(1,8)*PAVGM(8)*(1.+SW(22)*GLOB7S_2020(PMA(1,8)))
...
C      ***** N2 DENSITY ****
        DMR=DS(3)/DM28M-1.
        D(3)=
     &DENSM_2020(ALT,DM28M,XMM,TZ,MN3,ZN3,TN3,TGN3,MN2,ZN2,TN2,TGN2)
        D(3)=D(3)*(1.+DMR*DMC)
C      ***** HE DENSITY ****
        D(1)=0
...
C
   ...
   10 CONTINUE
      GOTO 90
   50 CONTINUE
      DD=DENSM_2020(ALT,1.,0.0,TZ,MN3,ZN3,TN3,TGN3,MN2,ZN2,TN2,TGN2)
      T(2)=TZ
   90 CONTINUE
      ALAST=ALT
      RETURN
...
C-----------------------------------------------------------------------
      DIMENSION D(9),T(2),AP(7),DS(9),TS(2)
      COMMON/METSEL/IMR
      CALL GTD7_2020(IYD,SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,MASS,D,T)
C       TOTAL MASS DENSITY
C
        IF(MASS.EQ.48) THEN
         D(6) = 1.66E-24*(4.*D(1)+16.*D(2)+28.*D(3)+32.*D(4)+40.*D(5)+
...
C
C
      SUBROUTINE GHP7_2020(IYD,SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,
     &  D,T,PRESS)
C-----------------------------------------------------------------------
...
C      ITERATION LOOP
   ...
   10 CONTINUE
        L=L+1
        CALL GTD7_2020(IYD,SEC,Z,GLAT,GLONG,STL,F107A,F107,AP,48,D,T)
...
C
C
      SUBROUTINE GLATF_2020(LAT,GV,REFF)
C-----------------------------------------------------------------------
...
C
C
      FUNCTION VTST7_2020(IYD,SEC,GLAT,GLONG,STL,F107A,F107,AP,IC)
C-----------------------------------------------------------------------
...
C        Test for changed input
      V2=VTST7_2020(IYD,SEC,GLAT,GLONG,STL,F107A,F107,AP,2)
C
      YRD=IYD
      ZA=PDL(16,2)
      ZN1(1)=ZA
...
C        VARIATIONS NOT IMPORTANT BELOW ZA OR ZN1(1)
     ...
     &  *(1.+
     &SW(16)*GLOBE7_2020(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PT))
...
C          GRADIENT VARIATIONS NOT IMPORTANT BELOW ZN1(5)
     ...
     &   *(1.+
     &SW(19)*GLOBE7_2020(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PS))
      ELSE
        G0=PTM(4)*PS(1)
      ENDIF
C      Calculate these temperatures only if input changed
      IF(V2.EQ.1. .OR. ALT.LT.300.)
     &  TLB=PTM(2)*(1.+SW(17)*GLOBE7_2020(YRD,SEC,GLAT,GLONG,STL,
...
C        density above 300 km
       IF(ALT.LT.300.) THEN
        IF(V2.EQ.1..OR.ALAST.GE.300.) THEN
         TN1(2)=PTM(7)*PTL(1,1)/(1.-SW(18)*GLOB7S_2020(PTL(1,1)))
         TN1(3)=PTM(3)*PTL(1,2)/(1.-SW(18)*GLOB7S_2020(PTL(1,2)))
         TN1(4)=PTM(8)*PTL(1,3)/(1.-SW(18)*GLOB7S_2020(PTL(1,3)))
         TN1(5)=PTM(5)*PTL(1,4)/(1.-SW(18)*SW(20)*GLOB7S_2020(PTL(1,4)))
         TGN1(2)=PTM(9)*PMA(1,9)*(1.+
     &SW(18)*SW(20)*GLOB7S_2020(PMA(1,9)))
...
C       N2 variation factor at Zlb
      G28=SW(21)*GLOBE7_2020(YRD,SEC,GLAT,GLONG,STL,F107A,F107,
     & AP,PDA3)
      DAY=AMOD(YRD,1000.)
...
C      Diffusive density at Alt
      D(3)=
     &DENSU_2020(Z,DB28,TINF,TLB, 28.,ALPHA(3),T(2),PTM(6),S,MN1,ZN1,
     & TN1,TGN1)
      DD=D(3)
C      Turbopause
      ZH28=PDM(3,3)*ZHF
...
C      Mixed density at Zlb
      B28=DENSU_2020(ZH28,DB28,TINF,TLB,XMD,ALPHA(3)-1.,TZ,PTM(6),S,MN1,
     & ZN1,TN1,TGN1)
      IF(Z.GT.ALTL(3).OR.SW(15).EQ.0.) GO TO 17
C      Mixed density at Alt
      DM28=DENSU_2020(Z,B28,TINF,TLB,XMM,ALPHA(3),TZ,PTM(6),S,MN1,
     & ZN1,TN1,TGN1)
C      Net density at Alt
      D(3)=DNET_2020(D(3),DM28,ZHM28,XMM,28.)
   17 CONTINUE
...
C       Density variation factor at Zlb
      G4 = SW(21)*GLOBE7_2020(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PDA1)
C      Diffusive density at Zlb
      DB04 = PDM(1,1)*EXP(G4)*PDA1(1)
C      Diffusive density at Alt
      D(1)=
     &DENSU_2020(Z,DB04,TINF,TLB, 4.,ALPHA(1),T(2),PTM(6),S,MN1,ZN1,
     & TN1,TGN1)
      DD=D(1)
...
C      Turbopause
      ZH04=PDM(3,1)
C      Mixed density at Zlb
      B04=DENSU_2020(ZH04,DB04,TINF,TLB,4.-XMM,ALPHA(1)-1.,
     &  T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
C      Mixed density at Alt
      DM04=DENSU_2020(Z,B04,TINF,TLB,XMM,0.,T(2),PTM(6),
     &S,MN1,ZN1,TN1,TGN1)
      ZHM04=ZHM28
C      Net density at Alt
      D(1)=DNET_2020(D(1),DM04,ZHM04,XMM,4.)
...
C      Net density corrected at Alt
      D(1)=D(1)*CCOR_2020(Z,RL,HC04,ZC04)
   24 CONTINUE
...
C      G16= SW(21)*GLOBE7(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PDA2)
      G16= SW(21)*GLOBE7_2020(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PDA2)
C      Diffusive density at Zlb
C      DB16 =  PDM(1,2)*EXP(G16)*PDA2(1)
      DB16 =  PDM(1,2)*EXP(G16)*PDA2(1)
C       Diffusive density at Alt
      D(2)=DENSU_2020(Z,DB16,TINF,TLB, 16.,ALPHA(2),T(2),PTM(6),S,MN1,
     & ZN1,TN1,TGN1)
      DD=D(2)
...
C       Turbopause
      ZH16=PDM(3,2)
C      Mixed density at Zlb
      B16=DENSU_2020(ZH16,DB16,TINF,TLB,16-XMM,ALPHA(2)-1.,
     &  T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
C      Mixed density at Alt
      DM16=DENSU_2020(Z,B16,TINF,TLB,XMM,0.,T(2),PTM(6),
     &S,MN1,ZN1,TN1,TGN1)
      ZHM16=ZHM28
C      Net density at Alt
      D(2)=DNET_2020(D(2),DM16,ZHM16,XMM,16.)
...
C
C       Density variation factor at Zlb
      G32= SW(21)*GLOBE7_2020(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PDA5)
C      Diffusive density at Zlb
      DB32 = PDM(1,4)*EXP(G32)*PDA5(1)
C       Diffusive density at Alt
      D(4)=DENSU_2020(Z,DB32,TINF,TLB, 32.,ALPHA(4),T(2),PTM(6),S,MN1,
     & ZN1,TN1,TGN1)
      IF(MASS.EQ.49) THEN
         DD=DD+2.*D(4)
      ELSE
...
C       Turbopause
      ZH32=PDM(3,4)
C      Mixed density at Zlb
      B32=DENSU_2020(ZH32,DB32,TINF,TLB,32.-XMM,ALPHA(4)-1.,
     &  T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
C      Mixed density at Alt
      DM32=DENSU_2020(Z,B32,TINF,TLB,XMM,0.,T(2),PTM(6),
     &S,MN1,ZN1,TN1,TGN1)
      ZHM32=ZHM28
C      Net density at Alt
      D(4)=DNET_2020(D(4),DM32,ZHM32,XMM,32.)
C       Correction to specified mixing ratio at ground
      RL=ALOG(B28*PDM(2,4)/B32)
      HC32=PDM(6,4)*PDL(8,2)
      ZC32=PDM(5,4)*PDL(7,2)
      D(4)=D(4)*CCOR_2020(Z,RL,HC32,ZC32)
   38 CONTINUE
...
C      Net density corrected at Alt
      D(4)=D(4)*CCOR2_2020(Z,RC32,HCC32,ZCC32,HCC232)
   39 CONTINUE
      IF(MASS.NE.48)   GO TO 90
   40 CONTINUE
C
...
C
C       Density variation factor at Zlb
      G40= SW(21)*GLOBE7_2020(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PDA6)
C      Diffusive density at Zlb
      DB40 = PDM(1,5)*EXP(G40)*PDA6(1)
C       Diffusive density at Alt
      D(5)=DENSU_2020(Z,DB40,TINF,TLB, 40.,ALPHA(5),T(2),PTM(6),S,MN1,
     & ZN1,TN1,TGN1)
      DD=D(5)
...
C       Turbopause
      ZH40=PDM(3,5)
C      Mixed density at Zlb
      B40=DENSU_2020(ZH40,DB40,TINF,TLB,40.-XMM,ALPHA(5)-1.,
     &  T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
C      Mixed density at Alt
      DM40=DENSU_2020(Z,B40,TINF,TLB,XMM,0.,T(2),PTM(6),
     &S,MN1,ZN1,TN1,TGN1)
      ZHM40=ZHM28
C      Net density at Alt
      D(5)=DNET_2020(D(5),DM40,ZHM40,XMM,40.)
C       Correction to specified mixing ratio at ground
      RL=ALOG(B28*PDM(2,5)/B40)
      HC40=PDM(6,5)*PDL(10,2)
      ZC40=PDM(5,5)*PDL(9,2)
C      Net density corrected at Alt
      D(5)=D(5)*CCOR_2020(Z,RL,HC40,ZC40)
   44 CONTINUE
      IF(MASS.NE.48)   GO TO 90
   45 CONTINUE
C
...
C
C       Density variation factor at Zlb
      G1 = SW(21)*GLOBE7_2020(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PDA7)
C      Diffusive density at Zlb
      DB01 = PDM(1,6)*EXP(G1)*PDA7(1)
C       Diffusive density at Alt
      D(7)=DENSU_2020(Z,DB01,TINF,TLB,1.,ALPHA(7),T(2),PTM(6),S,MN1,
     & ZN1,TN1,TGN1)
      DD=D(7)
...
C       Turbopause
      ZH01=PDM(3,6)
C      Mixed density at Zlb
      B01=DENSU_2020(ZH01,DB01,TINF,TLB,1.-XMM,ALPHA(7)-1.,
     &  T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
C      Mixed density at Alt
      DM01=DENSU_2020(Z,B01,TINF,TLB,XMM,0.,T(2),PTM(6),
     &S,MN1,ZN1,TN1,TGN1)
      ZHM01=ZHM28
C      Net density at Alt
      D(7)=DNET_2020(D(7),DM01,ZHM01,XMM,1.)
C       Correction to specified mixing ratio at ground
      RL=ALOG(B28*PDM(2,6)*ABS(PDL(18,2))/B01)
      HC01=PDM(6,6)*PDL(12,2)
      ZC01=PDM(5,6)*PDL(11,2)
      D(7)=D(7)*CCOR_2020(Z,RL,HC01,ZC01)
C       Chemistry correction
      HCC01=PDM(8,6)*PDL(20,2)
      ZCC01=PDM(7,6)*PDL(19,2)
      RC01=PDM(4,6)*PDL(21,2)
C      Net density corrected at Alt
      D(7)=D(7)*CCOR_2020(Z,RC01,HCC01,ZCC01)
   47 CONTINUE
      IF(MASS.NE.48)   GO TO 90
   48 CONTINUE
C
...
C       Density variation factor at Zlb
      G14 = SW(21)*
     &GLOBE7_2020(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PDA8)
C      Diffusive density at Zlb
      DB14 = PDM(1,7)*EXP(G14)*PDA8(1)
C       Diffusive density at Alt
      D(8)=DENSU_2020(Z,DB14,TINF,TLB,14.,ALPHA(8),T(2),PTM(6),S,MN1,
     & ZN1,TN1,TGN1)
      DD=D(8)
...
C       Turbopause
      ZH14=PDM(3,7)
C      Mixed density at Zlb
      B14=DENSU_2020(ZH14,DB14,TINF,TLB,14.-XMM,ALPHA(8)-1.,
     &  T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
C      Mixed density at Alt
      DM14=DENSU_2020(Z,B14,TINF,TLB,XMM,0.,T(2),PTM(6),
     &S,MN1,ZN1,TN1,TGN1)
      ZHM14=ZHM28
C      Net density at Alt
      D(8)=DNET_2020(D(8),DM14,ZHM14,XMM,14.)
C       Correction to specified mixing ratio at ground
      RL=ALOG(B28*PDM(2,7)*ABS(PDL(3,1))/B14)
      HC14=PDM(6,7)*PDL(2,1)
      ZC14=PDM(5,7)*PDL(1,1)
      D(8)=D(8)*CCOR_2020(Z,RL,HC14,ZC14)
C       Chemistry correction
      HCC14=PDM(8,7)*PDL(5,1)
      ZCC14=PDM(7,7)*PDL(4,1)
      RC14=PDM(4,7)*PDL(6,1)
C      Net density corrected at Alt
      D(8)=D(8)*CCOR_2020(Z,RC14,HCC14,ZCC14)
   49 CONTINUE
      IF(MASS.NE.48) GO TO 90
   46 CONTINUE
C
...
C
      G16H = SW(21)*
     &GLOBE7_2020(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PDA9)
      DB16H = PDM(1,8)*EXP(G16H)*PDA9(1)
      THO=PDM(10,8)*PDL(7,1)
      DD=DENSU_2020(Z,DB16H,THO,THO,16.,ALPHA(9),T2,PTM(6),S,MN1,
     & ZN1,TN1,TGN1)
      ZSHT=PDM(6,8)
      ZMHO=PDM(5,8)
      ZSHO=SCALH_2020(ZMHO,16.,THO)
...
C       TEMPERATURE AT ALTITUDE
   50 CONTINUE
      Z=ABS(ALT)
      DDUM  =
     &DENSU_2020(Z,1., TINF,TLB,0.,0.,T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
   90 CONTINUE
...
C
C
      FUNCTION SCALH_2020(ALT,XM,TEMP)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      DATA RGAS/831.4/
      G=GSURF/(1.+ALT/RE)**2
      scalh_2020=RGAS*TEMP/(G*XM)
      RETURN
      END
C
C
      FUNCTION GLOBE7_2020(YRD,SEC,LAT,LONG,TLOC,F107A,F107,AP,P)
C-----------------------------------------------------------------------
...
C
C
      FUNCTION DENSU_2020(ALT,DLB,TINF,TLB,XM,ALPHA,TZ,ZLB,S2,
     &  MN1,ZN1,TN1,TGN1)
C--------------------------------------------------------------------
...
C       Calculate spline coefficients
      CALL SPLINEM_2020(XS,YS,MN,YD1,YD2,Y2OUT)
      X=ZG/ZGDIF
      CALL SPLINTM_2020(XS,YS,Y2OUT,MN,X,Y)
C       temperature at altitude
      TZ=1./Y
      densu_2020=TZ
...
C       integrate spline temperatures
      CALL SPLINI_2020(XS,YS,Y2OUT,MN,X,YI)
      EXPL=GAMM*YI
      IF(EXPL.GT.50..OR.TZ.LE.0.) THEN
        EXPL=50.
      ENDIF
C       Density at altitude
      densu_2020=DENSU*(T1/TZ)**(1.+ALPHA)*EXP(-EXPL)
   50 CONTINUE
      RETURN
      END
C
C
      FUNCTION DENSM_2020(
     &ALT,D0,XM,TZ,MN3,ZN3,TN3,TGN3,MN2,ZN2,TN2,TGN2)
C--------------------------------------------------------------------
...
C       Density at altitude
      densm_2020=DENSM*(T1/TZ)*EXP(-EXPL)
   20 CONTINUE
...
C        Density at altitude
   ...
   30 CONTINUE
   50 CONTINUE
      IF(XM.EQ.0) densm_2020=TZ
      RETURN
      END
C
C
      SUBROUTINE SPLINEM_2020(X,Y,N,YP1,YPN,Y2)
C-----------------------------------------------------------------------
...
C
C
      SUBROUTINE SPLINTM_2020(XA,YA,Y2A,N,X,Y)
C-----------------------------------------------------------------------
...
C
C
      SUBROUTINE SPLINI_2020(XA,YA,Y2A,N,X,YI)
C-----------------------------------------------------------------------
...
C
C
      FUNCTION DNET_2020(DD,DM,ZHM,XMM,XM)
C-----------------------------------------------------------------------
...
C
C
      FUNCTION  CCOR_2020(ALT, R,H1,ZH)
C-----------------------------------------------------------------------
...
C
C
      FUNCTION  CCOR2_2020(ALT, R,H1,ZH,H2)
C-----------------------------------------------------------------------
