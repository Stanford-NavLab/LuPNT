// Lunar frozen-orbit study. The example propagates several candidate orbits,
// caches long runs to HDF5, and writes diagnostic plots under the output folder.
...
private:
  Real t0_;
  State coe0_moon_op_;
  KeplerianDynamics<ClassicalOE> dyn_moon_;

public:
  Case1Dynamics(Real t0, State coe0_moon_op)
      : CartesianTwoBodyDynamics(GM_MOON),
        t0_(t0),
        coe0_moon_op_(coe0_moon_op),
        dyn_moon_(GM_EARTH) {
    SetODE([this](Real t, const State& rv) { return ComputeRates(t, rv); });
  }

  Vec6 ComputeRates(Real t, const State& rv) {
    Vec6 coe_moon_op = dyn_moon_.Propagate(coe0_moon_op_, t0_, t);
    Vec6 rv_earth_op = -ClassicalToCart(coe_moon_op, GM_EARTH);
...
int main() {
  ...
  auto output_path = GetOutputDir("ex_frozen_orbits");
  ...
  cout << "Start epoch       " << TimeToGregorianString(t0) << endl;
  cout << "End epoch         " << TimeToGregorianString(t0 + dt_total) << endl;
  ...
  // Propagate
  MatX6 rv_case0_ci = dyn0.Propagate(rv0_ci, tspan);
  ...
  tspan = Arange(Real(0.0), dt_total + dt_step, dt_step);  // [s] Time span
  ...
  // Initial state
  Vec6 rv0_moon_op = GetBodyPosVel(t0, BodyId::EARTH, BodyId::MOON, Frame::MOON_OP);
  Vec6 coe0_moon_op = CartToClassical(rv0_moon_op, GM_EARTH);

  // Dynamics
  KeplerianDynamics dyn_moon(GM_EARTH);

  // Propagate
  MatX6 coe_moon_op = dyn_moon.Propagate(coe0_moon_op, tspan);
  MatX6 rv_moon_op = ClassicalToCart(coe_moon_op, GM_EARTH);
  ...
  if (config.plot_case1) {
    ...
    hold(true);
    PlotBody(BodyId::EARTH);
    PlotBody(BodyId::MOON, rv0_moon_op.head(3));
    p = Plot3(rv_moon_op.col(0), rv_moon_op.col(1), rv_moon_op.col(2), "b-o");
    p->marker_indices({0});
    p->line_width(2);
    SetLim(D_EARTH_MOON * 1.1);
    fig->save(output_path / "case1.png");
  }

  // Dynamics
  Case1Dynamics dyn_3body_circ(t0, coe0_moon_op);
  dyn_3body_circ.SetIntegrator(IntegratorType::RK4);
  dyn_3body_circ.SetTimeStep(dt_prop);

  // Propagate
  MatX6 rv_case1_op;
  if (config.recompute_part1 || !file_part1.exist("/rv_case1_op")) {
    rv_case1_op = dyn_3body_circ.Propagate(rv0_op, tspan);
    dump(file_part1, "/rv_case1_op", rv_case1_op.cast<double>().eval(),
         H5Easy::DumpMode::Overwrite);
  } else {
    rv_case1_op = H5Easy::load<MatX6d>(file_part1, "/rv_case1_op");
  }
  MatX6 coe_case1_op = CartToClassical(rv_case1_op, GM_MOON);

  int n_plot = 20;
  VecX tspan_plot = Arange(Real(0.0), sat_period + dt_prop, dt_prop);
  if (config.plot_case1) {
    ...
    xlim({0, dt_total.val() / SECS_DAY});
    grid(true);
    fig->save(output_path / "case1_oe.png");

    // Plot orbits
    vector<MatX6> rv_plot_op;
    for (int i = 0; i < n_plot; ++i) {
      int j = i * n_steps / n_plot;
      VecX tfs_plot = tspan[j] + tspan_plot.array();
      Vec6 rv0_plot = rv_case1_op.row(j);
      rv_plot_op.push_back(dyn_3body_circ.Propagate(rv0_plot, tfs_plot));
    }
    fig = figure(true);
    ...
    SetLim(12e3);
    fig->save(output_path / "case1_orbit.png");
  }
  ...
  // Dynamics
  NBodyDynamics dyn_3body;
  dyn_3body.SetIntegrator(IntegratorType::RK4);
  ...
  if (config.recompute_part1 || !file_part1.exist("/rv_case2_ci")) {
    rv_case2_ci = dyn_3body.Propagate(rv0_ci, tspan);
    H5Easy::dump(file_part1, "/rv_case2_ci", rv_case2_ci.cast<double>().eval(),
                 H5Easy::DumpMode::Overwrite);
  } else {
  ...
  MatX6 coe_case2_op = CartToClassical(rv_case2_op, GM_MOON);
  ...
  NBodyDynamics dyn_nbody;
  dyn_nbody.SetIntegrator(IntegratorType::RK4);
  ...
  dyn_nbody.SetPrintProgress(true);

  // Propagate
  MatX6 rv_case3_ci;
  if (config.recompute_part1 || !file_part1.exist("/rv_case3_ci")) {
    rv_case3_ci = dyn_nbody.Propagate(rv0_ci, tspan);
    H5Easy::dump(file_part1, "/rv_case3_ci", rv_case3_ci.cast<double>().eval(),
                 H5Easy::DumpMode::Overwrite);
  } else {
  ...
  MatX6 coe_case3_op = CartToClassical(rv_case3_op, GM_MOON);
  ...
  MatX6 coe_case4_me = CartToClassical(rv_case4_me, GM_MOON);
  ...
  dyn_nbody50.SetPrintProgress(true);

  // Propagate
  MatX6 rv_case5_ci;
  if (config.recompute_part1 || !file_part1.exist("/rv_case5_ci")) {
    rv_case5_ci = dyn_nbody50.Propagate(rv0_ci, tspan);
    H5Easy::dump(file_part1, "/rv_case5_ci", rv_case5_ci.cast<double>().eval(),
                 H5Easy::DumpMode::Overwrite);
  } else {
  ...
  MatX6 coe_case5_me = CartToClassical(rv_case5_me, GM_MOON);
  ...
  if (config.recompute_part1 || !file_part1.exist("/rv_case6_ci")) {
    rv_case6_ci = dyn_nbody.Propagate(rv0_ci, tspan);
    H5Easy::dump(file_part1, "/rv_case6_ci", rv_case6_ci.cast<double>().eval(),
                 H5Easy::DumpMode::Overwrite);
  } else {
  ...
  MatX6 coe_case6_me = CartToClassical(rv_case6_me, GM_MOON);
  ...
  if (config.plot_ew) {
    ...
    fig->save(output_path / "ew_plots.png");
  }
  ...
  for (int i = 0; i < n_sat; ++i) {
    if (config.recompute_part2 || !file_part2.exist("/rvs_ci" + to_string(i))) {
      ...
      Vec6 rv0_op_ = ClassicalToCart(coe0_op_, GM_MOON);
      Vec6 rv0_ci_ = ConvertFrame(t0, rv0_op_, Frame::MOON_OP, Frame::MOON_CI);
      rvs_ci.push_back(dyn_nbody.Propagate(rv0_ci_, tspan));
      H5Easy::dump(file_part2, "/rvs_ci" + to_string(i), rvs_ci[i].cast<double>().eval(),
                   H5Easy::DumpMode::Overwrite);
    } else {
  ...
  for (int i = 0; i < n_sat; ++i) coes_ci.push_back(CartToClassical(rvs_ci[i], GM_MOON));
  ...
  if (config.plot_delta_M) {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      hold(true);
      VecX delta_M_tmp = WrapToPi(coes_ci[i + 1].col(5) - coes_ci[0].col(5)) * DEG;
      VecX delta_M(delta_M_tmp.size());
      delta_M[0] = delta_M_tmp[0];
      for (int j = 1; j < delta_M.size(); ++j)
        delta_M[j] = delta_M[j - 1] + WrapToPi(delta_M_tmp[j] - delta_M_tmp[j - 1]);
    ...
    fig->save(output_path / "delta_M.png");
  }
  ...
  for (int i = 0; i < n_sat; ++i) {
    if (config.recompute_part2 || !file_part2.exist("/rvs_ci_adjusted" + to_string(i))) {
      ...
      Vec6 rv0_op_i = ClassicalToCart(coe0_op_, GM_MOON);
      Vec6 rv0_ci_i = ConvertFrame(t0, rv0_op_i, Frame::MOON_OP, Frame::MOON_CI);
      rvs_ci_adjusted.push_back(dyn_nbody.Propagate(rv0_ci_i, tspan));
      H5Easy::dump(file_part2, "/rvs_ci_adjusted" + to_string(i),
                   rvs_ci_adjusted[i].cast<double>().eval(), H5Easy::DumpMode::Overwrite);
    } else {
  ...
  for (int i = 0; i < n_sat; ++i) {
    coes_ci_adjusted.push_back(CartToClassical(rvs_ci_adjusted[i], GM_MOON));
  }
  ...
  if (config.plot_delta_M) {
    ...
    for (int i = 0; i < 2; ++i) {
      fig->add_subplot(1, 2, i);
      hold(true);
      VecX delta_M = WrapToPi(coes_ci_adjusted[i + 1].col(5) - coes_ci_adjusted[0].col(5)) * DEG;
      if (i == 1) {
    ...
    fig->save(output_path / "delta_M_adjusted.png");
  }
  ...
  // Elevation
  Vec3 r_south_pole = LatLonAltToCart(Vec3(-90 * RAD, 0, 0), R_MOON);
  MatXi visibility = MatXi::Zero(n_sat, n_steps);
  for (int i = 0; i < n_sat; ++i) {
    VecX elev = CartToAzElRange(rs_me[i], r_south_pole).col(1);
