// Compares pass-by-value and pass-by-const-reference patterns for small Eigen
// vectors.
...
// (pass-by-value)
Vec3 GeocentricToCartValue(Vec3 r_geo, real radius) {
  real r = radius;
...
Vec3 GeocentricToCartRef(const Vec3 &r_geo, real radius) {
  real r = radius;
  real theta = r_geo(1);  // Latitude
  real phi = r_geo(2);    // Longitude
