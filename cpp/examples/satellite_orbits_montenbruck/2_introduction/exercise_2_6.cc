// Montenbruck Exercise 2-6: estimate an initial orbit from two topocentric
// angle/range observations.
...
int main() {
  // Ground station
  const Vec3 r_gs(1344.143e3, 6068.601e3, 1429.311e3);       // (x, y, z) [m]
  const Vec3 lla = CartToLatLonAlt(r_gs, R_EARTH, WGS84_F);  // (lat, lon, alt) [rad, rad, m]
  ...
  cout << "  Semimajor axis   " << setw(10) << coe(0) / 1000.0 << " m" << endl;
  cout << setprecision(7);
