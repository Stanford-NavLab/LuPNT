...
namespace {
  // Small, trimmed RINEX-nav (BRDC) file committed under `cpp/test/fixtures/gnss/`
  // (full header + a few GPS G01/G02/G03 records bracketing 2026-01-14 01:00, plus
  // one GLONASS record) so the loader gets CI coverage without the ~12 MB product.
...
TEST_CASE("interfaces.rinex_nav_fixture") {
  const std::filesystem::path nav_file = FixtureFile();
