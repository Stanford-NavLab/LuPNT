...
namespace {
  ...
  void RequireNearMat3d(const Mat3d& a, const Mat3d& b, double tol) {
    for (int i = 0; i < 3; i++)
      for (int j = 0; j < 3; j++) REQUIRE_THAT(a(i, j), WithinAbs(b(i, j), tol));
  }
}  // namespace

const double epsilon = 1e-6;

TEST_CASE("interfaces.antex_loader") {
  ...
  // (`output/gnss_files/sp3/COD0MGXFIN_20260140000_01D_05M_ORB.SP3`, first epoch).
