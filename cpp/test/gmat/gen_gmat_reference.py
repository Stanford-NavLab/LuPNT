...
pixi/conda. To run this script you need a local GMAT R2022a (or compatible)
...
IMPORTANT (GMAT R2022a bug workaround): each generated `Propagate` command
...
found to desync the second spacecraft's force model (J2Sat ended up ~60x
further from the two-body solution than the expected J2 perturbation).

See `cpp/test/gmat/README.md` for the JSON schema, the GMAT-specific
limitations (no UT1/GPS time scales, custom potential file, DE421 vs DE440
ephemerides, etc.), and the tolerance rationale used by the
`*_gmat_reference` tests.
"""
...
def find_gmat_console() -> Path:
    ...
    raise FileNotFoundError(
        ...
        "/Applications/GMAT_R2022a/bin/GmatConsole-R2022a), or GMAT_HOME to "
...
def build_script(cof_path: str, rf_paths: dict[str, str]) -> str:
    ...
    s.append(
        f"""%----------------------------------------------------------------------------
% LuPNT cross-validation reference script (GMAT R2022a)
%
...
% command was found to desync the second spacecraft's force model in GMAT
% R2022a. Always use separate sequential `Propagate` commands.
%----------------------------------------------------------------------------

% DE421 is the closest JPL ephemeris to LuPNT's DE440 that ships with GMAT
% R2022a (the Moon-frame comparisons account for the remaining DE421 vs
% DE440 difference -- see cpp/test/gmat/README.md).
...
def main() -> None:
    ...
    out: dict = {
        "_meta": {
            ...
            "notes": [
                "GMAT R2022a does not expose a UT1 or GPS time scale as a "
                ...
                "outputs.",
                "GMAT R2022a ships DE405/DE421/DE424 (no DE440), so the "
                "`moon_frames` section was generated with DE421; the "
                "Moon-centered comparisons absorb the remaining DE421 vs "
                "DE440 lunar-ephemeris difference. `r_moon_fixed` is GMAT's "
                ...
                "meters after 0.25-1.5 orbits, because LuPNT evaluates the "
                "J2 acceleration using the inertial-frame Z axis as the "
                "symmetry axis, whereas GMAT's GravityField evaluates the "
                "spherical harmonics in the Earth body-fixed frame. See "
                "cpp/test/gmat/README.md for details.",
            ],
        }
    }
