...
def summary(valobj, internalr_dict, options):
    ...
    filtered_values = [v for n, v in zip(names, values) if n not in ("m_rows", "m_cols")]
    ...
    summary_txt += print_aligned(array, all_rows=rows == n_rows, all_columns=cols == n_cols)
    # summary_txt += "\n" + str(np.array(array))
    return summary_txt
...
class MatChildProvider:
    ...
    def get_child_at_index(self, index):
        ...
        if not self._fixed_cols:
            if (index == 0 and not self._fixed_rows) or (index == 1 and self._fixed_rows):
