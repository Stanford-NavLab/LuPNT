...
class TestOrbitState:
    ...
    def test_ClassicalOE(self):
        ...
        coe_state = pnt.ClassicalOE(coe, pnt.Frame.MOON_CI)
        attributes = ["a", "e", "i", "Omega", "w", "M"]

        # Getters
        for i, attr in enumerate(attributes):
            assert getattr(coe_state, attr) == coe[i]
        assert coe_state.frame == pnt.Frame.MOON_CI
    ...
    def test_CartesianOrbitState(self):
        ...
        cart_state = pnt.CartesianOrbitState(rv, pnt.Frame.MOON_CI)

        # Getters
        assert np.allclose(cart_state.r, rv[0:3])
        assert np.allclose(cart_state.v, rv[3:6])

        assert cart_state.frame == pnt.Frame.MOON_CI
    ...
    def test_QuasiNonsingOE(self):
        ...
        assert qns_oe_state.frame == pnt.Frame.MOON_CI
    ...
    def test_EquinoctialOE(self):
        ...
        eq_oe_state = pnt.EquinoctialOE(eq_oe, pnt.Frame.MOON_CI)
        attributes = ["a", "h", "k", "p", "q", "lon"]

        # Getters
        for i, attr in enumerate(attributes):
            assert getattr(eq_oe_state, attr) == eq_oe[i]
        assert eq_oe_state.frame == pnt.Frame.MOON_CI
    ...
    def test_SingularROE(self):
        ...
        roe_state = pnt.SingularROE(roe, pnt.Frame.MOON_CI)
        attributes = ["ada", "adM", "ade", "adw", "adi", "adOmega"]

        # Getters
        for i, attr in enumerate(attributes):
            assert getattr(roe_state, attr) == roe[i]
        assert roe_state.frame == pnt.Frame.MOON_CI
