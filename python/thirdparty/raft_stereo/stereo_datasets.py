...
class SceneFlowDatasets(StereoDataset):
    ...
    def _add_things(self, split="TRAIN"):
        ...
        disparity_images = [
            im.replace(self.dstype, "disparity").replace(".png", ".pfm") for im in left_images
        ]
        ...
        if split == "2014":  # datasets/Middlebury/2014/Pipes-perfect/im0.png
            ...
            lines = list(
                filter(
                    lambda p: any(
                        s in p.split("/")
                        for s in Path(os.path.join(root, "MiddEval3/official_train.txt"))
                        .read_text()
                        .splitlines()
                    ),
                    lines,
            ...
            image1_list = sorted(
                [
                    os.path.join(root, "MiddEval3", f"training{split}", f"{name}/im0.png")
                    for name in lines
                ]
            )
            image2_list = sorted(
                [
                    os.path.join(root, "MiddEval3", f"training{split}", f"{name}/im1.png")
                    for name in lines
                ]
            )
            disp_list = sorted(
                [
                    os.path.join(root, "MiddEval3", f"training{split}", f"{name}/disp0GT.pfm")
                    for name in lines
                ]
            )
