"""Shared plotting helpers for the frozen-orbit notebooks."""
...
def plot_mean_anomaly_differences(tfs: np.ndarray, rvs: np.ndarray, set_lims=True):
    """Plot pairwise mean-anomaly drift for three propagated spacecraft."""

    coes = np.zeros_like(rvs)
    for i in range(3):
        coes[i] = pnt.cart_to_classical(rvs[i], pnt.GM_MOON)
