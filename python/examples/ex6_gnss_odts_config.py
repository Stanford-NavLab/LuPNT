...
# Agent-based scenario config (world: + a LunarGnssManager hosting a LunarGnssOdtsApp). The
...
def load_scenario():
    ...
    Returns the full scenario dict (top-level ``world:`` + a ``gnss_manager`` agent hosting a
    ...
    app = scen["agents"]["gnss_manager"]["application"]
