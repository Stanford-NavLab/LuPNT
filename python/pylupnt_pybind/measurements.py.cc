...
void init_measurements(py::module &m) {
  m.def(
      "compute_occultation",
      [](double epoch, const Vector3d &r1, const Vector3d &r2, CoordSystem cs1,
         CoordSystem cs2, const std::vector<NaifId> &bodies) -> VectorXd {
  ...
  m.def(
      "compute_occultation",
      [](double epoch, Matrixd<-1, 3> &r1, Matrixd<-1, 3> &r2, CoordSystem cs1,
         CoordSystem cs2, const std::vector<NaifId> &bodies) -> MatrixXd {
        return Occultation::ComputeOccultation(epoch, r1.cast<real>().eval(),
                                               r2.cast<real>().eval(), cs1, cs2,
                                               bodies)
            .cast<double>();
  ...
  m.def(
      ...
      [](const VectorXd &epoch, const Matrixd<-1, 3> &r1,
         const Matrixd<-1, 3> &r2, CoordSystem cs1, CoordSystem cs2,
