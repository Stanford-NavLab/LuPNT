  ...
  py::class_<CraterDataLoader>(m, "CraterDataLoader")
      .def_static(
          ...
          [](const std::string& filename,
             std::optional<std::pair<double, double>> latlims,
             std::optional<std::pair<double, double>> longlims,
             std::pair<double, double> diamlims, double ellipse_limit,
             double arc_lims) {
            const std::pair<double, double>* latlims_ptr =
                latlims ? &*latlims : nullptr;
            const std::pair<double, double>* longlims_ptr =
                longlims ? &*longlims : nullptr;
            return CraterDataLoader::LoadCraters(filename, latlims_ptr,
                                                 longlims_ptr, diamlims,
          ...
          py::arg("diamlims") = std::make_pair(0.0, 500.0),
          py::arg("ellipse_limit") = 1.5, py::arg("arc_lims") = 0.0,
          "Load craters from the dataset")
      .def_static(
          ...
          [](const std::vector<Crater>& craters, bool radians) {
            ...
            CraterDataLoader::ExtractRobbinsDataset(
                craters, lat, lon, major, minor, psi, crater_id, radians);
            return py::make_tuple(lat, lon, major, minor, psi, crater_id);
          },
          py::arg("craters"), py::arg("radians") = true,
          "Extract the Robbins dataset");
