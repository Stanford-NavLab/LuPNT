...
from ..common.camera import camera_matrix
from ..common.conics import conic_matrix, conic_center
from ..common.coordinates import nadir_attitude
from ..matching.position_estimation import PositionRegressor
from ..matching.projective_invariants import CoplanarInvariants
from ..matching.utils import get_cliques_by_length, shift_nd
...
class CraterDatabase:
    def __init__(
        ...
        """
        self._adjacency_matrix = radius_neighbors_graph(
            np.array([x, y, z]).T,
            radius,
            mode="distance",
            metric="euclidean",
        ...
        self._graph = nx.from_scipy_sparse_array(self._adjacency_matrix)
        ...
        cw_value = LA.det(
            np.moveaxis(
                np.array(
                    [
                        [r_i[:, 0], r_i[:, 1], np.ones_like(r_i[:, 0])],
                        [r_j[:, 0], r_j[:, 1], np.ones_like(r_i[:, 0])],
                        [r_k[:, 0], r_k[:, 1], np.ones_like(r_i[:, 0])],
                    ]
                ),
                -1,
                0,
        ...
        crater_triads[np.argwhere(~clockwise), [0, 1]] = crater_triads[
            np.argwhere(~clockwise), [1, 0]
        ]
        ...
        invariants = CoplanarInvariants(
            crater_triads, A_i, A_j, A_k, normalize_det=True
        )
        ...
        if sort_ij:
            ...
            too_close = np.logical_or.reduce(
                (
                    np.abs(
                        (self._features[:, 0] - self._features[:, 2])
                        / self._features[:, 0]
                    )
                    < 0.1,
                    np.abs(
                        (self._features[:, 0] - self._features[:, 1])
                        / self._features[:, 0]
                    )
                    < 0.1,
                )
        ...
        **kwargs
        ...
        if column_keys is None:
            column_keys = dict(
                lat="LAT_ELLI_IMG",
                long="LON_ELLI_IMG",
                major="DIAM_ELLI_MAJOR_IMG",
                minor="DIAM_ELLI_MINOR_IMG",
                angle="DIAM_ELLI_ANGLE_IMG",
                id="CRATER_ID",
            )
    ...
    def query(self, key, k=1, return_distance=False, max_distance=0.1, batch_size=100):
        ...
        if key.shape[-1] == 3:
            ...
            crater_triads, features = next(
                CoplanarInvariants.match_generator(
                    A_craters=key, max_iter=1, batch_size=batch_size
                )
            )
            ...
            dist = np.abs(
                (features[:, None, :] - self._features[match_idxs])
                / features[:, None, :]
            ).mean(-1)
            dist_filter = dist < max_distance
            r_query, C_query = self[match_idxs[dist_filter]]
            A_query = key[crater_triads[dist_filter]]
            A_query, r_query, C_query = (
                A_query.reshape(-1, 3, 3),
                r_query.reshape(-1, 3, 1),
                C_query.reshape(-1, 3, 3),
            )
    ...
    def query_position(
        ...
        **kwargs
        ...
        A_query, r_query, C_query = self.query(
            A_detections, k=k, max_distance=max_distance, batch_size=batch_size
        )
        estimator = PositionRegressor(
            sigma_pix=sigma_pix,
