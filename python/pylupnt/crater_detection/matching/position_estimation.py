...
class PositionRegressor:
    def __init__(self, sigma_pix=6, **ransac_kwargs):
        kwargs = dict(
            residual_threshold=0.01, max_trials=1000, is_model_valid=_model_validator()
        )
    ...
    def fit(
        self, A_query, C_query, r_query, attitude, camera_matrix, reprojection=True
    ):
        ...
        if len(A_query) > 4:
            ...
            B_craters = (
                attitude @ camera_matrix.T @ A_query @ camera_matrix @ LA.inv(attitude)
            )
            ...
            self.inlier_mask = np.logical_and.reduce(
                self.ransac.inlier_mask_.reshape(y_.shape), axis=(-1, -2)
            )
            ...
            if (
                self.num_inliers > 2
                and reprojection
                and self.ransac.is_model_valid(self.ransac.estimator_)
            ):
                C_inlier, r_inlier = (
                    C_query[self.inlier_mask],
                    r_query[self.inlier_mask],
                )
                ...
                if self.reprojection_mask.sum() > 2:
                    ...
                    with warnings.catch_warnings():
                        ...
                        self.optimize_result = least_squares(
                            lambda x: (
                                (y_verified / np.mean(y_verified))
                                - (X_verified / np.mean(y_verified)) @ x[:, None]
                            ).squeeze(),
