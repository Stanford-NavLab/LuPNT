...
from ..common.conics import matrix_adjugate, scale_det, conic_matrix, conic_center
...
class PermutationInvariant:
    ...
    @staticmethod
    def F3(x, y, z):
        return (-3 * np.sqrt(3) * (x - y) * (y - z) * (z - x)) / (
            x**2 + y**2 + z**2 - (x * y + y * z + z * x)
        )
    ...
    def G1(x1, y1, z1, x2, y2, z2):
        return 1.5 * (x1 * x2 + y1 * y2 + z1 * z2) - 0.5 * (x1 + y1 + z1) * (
            x2 + y2 + z2
        )
    ...
    def G2(x1, y1, z1, x2, y2, z2):
        return (np.sqrt(3) / 2.0) * (
            (x1 * z2 + y1 * x2 + z1 * y2) - (x1 * y2 + y1 * z2 + z1 * x2)
        )
    ...
    def G(cls, x1, y1, z1, x2, y2, z2):
        return np.array(
            (cls.G1(x1, y1, z1, x2, y2, z2), cls.G2(x1, y1, z1, x2, y2, z2))
        )
    ...
    def G_tilde(cls, x1, y1, z1, x2, y2, z2):
        ...
        """
        return cls.G(x1, y1, z1, x2, y2, z2) / np.sqrt(
            np.sqrt(
                (
                    ((x1 - y1) ** 2 + (y1 - z1) ** 2 + (z1 - x1) ** 2)
...
class CoplanarInvariants:
    def __init__(self, crater_triads, A_i, A_j, A_k, normalize_det=True):
        ...
        A_i, A_j, A_k = map(
            lambda craters_: craters_[~overlapped_craters], (A_i, A_j, A_k)
        )
        ...
        self.I_ij, self.I_ji = np.trace(
            LA.inv(A_i) @ A_j, axis1=-2, axis2=-1
        ), np.trace(LA.inv(A_j) @ A_i, axis1=-2, axis2=-1)
        self.I_ik, self.I_ki = np.trace(
            LA.inv(A_i) @ A_k, axis1=-2, axis2=-1
        ), np.trace(LA.inv(A_k) @ A_i, axis1=-2, axis2=-1)
        self.I_jk, self.I_kj = np.trace(
            LA.inv(A_j) @ A_k, axis1=-2, axis2=-1
        ), np.trace(LA.inv(A_k) @ A_j, axis1=-2, axis2=-1)
        self.I_ijk = np.trace(
            (matrix_adjugate(A_j + A_k) - matrix_adjugate(A_j - A_k)) @ A_i,
            axis1=-2,
            axis2=-1,
        )
    ...
    def from_detection_conics(cls, A_craters, crater_triads=None):
        ...
        A_i, A_j, A_k = np.array(
            list(map(lambda vertex: A_craters[vertex], crater_triads_cw.T))
        )
    ...
    def match_generator(
        ...
        if A_craters is not None:
            ...
            raise ValueError(
                "No detections provided! Use either parameterized ellipse values or conics as input."
            )
        ...
        if batch_size == 1:
            for it, (i, j, k) in enumerate(enhanced_pattern_shifting(n_det)):
                ...
                A_i, A_j, A_k = np.array(
                    list(map(lambda vertex: A_craters[vertex], crater_triad))
                )
            ...
            for it in range(n_comb // batch_size):
                ...
                crater_triads_cw = crater_triads.copy()
                crater_triads_cw[~clockwise] = np_swap_columns(
                    crater_triads[~clockwise]
                )
                x_triads[:, ~clockwise] = np_swap_columns(x_triads.T[~clockwise]).T
                y_triads[:, ~clockwise] = np_swap_columns(y_triads.T[~clockwise]).T
        ...
        if permutation_invariant:
            out = np.column_stack(
                (
                    ...
                    PermutationInvariant.G_tilde(
                        self.I_ij, self.I_jk, self.I_ki, self.I_ji, self.I_kj, self.I_ik
                    ).T,
                    self.I_ijk,
                )
            ).T
