...
def download_kernel(
    file_path,
    base_url=const.SPICE_BASE_URL,
    base_folder=const.KERNEL_ROOT,
    verbose=False,
):
...
def setup_spice(
    kernels=None, base_url=const.SPICE_BASE_URL, base_folder=const.KERNEL_ROOT
):
    if kernels is None:
        kernels = KERNELS
...
def get_sol_incidence(date, r):
    et = spice.str2et(str(date))
    _, _, _, sol_incidence, _ = spice.ilumin(
        "ellipsoid", "moon", et, "iau_moon", "lt+s", "sun", r
    )
