...
class SurRenderer(Camera):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.__setup_backend(
            DEM_filename=DEM_filename, texture_filename=texture_filename
        )
        self.__sync_backend()

    def __setup_backend(
        self, DEM_filename="FullMoon.dem", texture_filename="lroc_color_poles.tiff"
    ):
        ...
        self.backend.setObjectAttitude(
            "moon", Rotation.from_euler("z", np.pi, degrees=False).as_quat()
        )
    ...
    def __sync_backend(self):
        ...
        self.backend.setObjectAttitude(
            "camera", Rotation.from_matrix(self.attitude).as_quat()
        )
        self.backend.setObjectPosition("earth", self.earth_pos * 1e3)
        self.backend.setObjectPosition("sun", self.sun_pos * 1e3)
