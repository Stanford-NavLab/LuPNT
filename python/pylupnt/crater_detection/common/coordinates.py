...
class OrbitingBodyBase:
    ...
    def __init__(self, position=None, attitude=None, primary_body_radius=const.RMOON):
    ...
    def set_coordinates(
        self, lat, long, height=None, point_nadir=False, convert_to_radians=True
    ):
        if height is None:
            height = self.height
        ...
        self.position = np.array(
            spherical_to_cartesian(self._primary_body_radius + height, lat, long)
        )
    ...
    def set_random_position(self, min_height=150, max_height=400, height=None):
        ...
        self.set_coordinates(
            lat, long, height, point_nadir=True, convert_to_radians=True
        )
    ...
    def attitude(self, attitude: Union[np.ndarray, Rotation]):
        ...
        if attitude is None:
            ...
            if not np.isclose(abs(LA.det(attitude)), 1):
                raise ValueError(
                    f"Invalid rotation matrix! Determinant should be +-1, is {LA.det(attitude)}."
                )
    ...
    def coordinates(self):
        return tuple(
            map(lambda x: x.value.item(), cartesian_to_spherical(*self.position))
        )
    ...
    def height(self, height):
        ...
        self.position = (self.position / LA.norm(self.position)) * (
            self._primary_body_radius + height
        )

    def rotate(
        self, axis: str, angle: float, degrees: bool = True, reset_first: bool = False
    ):
        ...
        self.attitude = (
            Rotation.from_matrix(self.attitude)
            * Rotation.from_euler(axis, angle, degrees=degrees)
        ).as_matrix()
