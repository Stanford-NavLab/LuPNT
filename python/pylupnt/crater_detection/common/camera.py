...
def camera_matrix(fov=const.CAMERA_FOV, resolution=const.CAMERA_RESOLUTION, alpha=0):
    ...
    if isinstance(fov, Iterable):
        f_x, f_y = map(
            lambda num, fov_: num / np.tan(np.radians(fov_ / 2)), (x_0, y_0), fov
        )
    else:
        f_x, f_y = map(lambda num: num / np.tan(np.radians(fov / 2)), (x_0, y_0))

    return np.array([[f_x, alpha, x_0], [0, f_y, y_0], [0, 0, 1]])
...
class Camera(OrbitingBodyBase):
    ...
    def __init__(
        self, fov=const.CAMERA_FOV, resolution=const.CAMERA_RESOLUTION, **kwargs
    ):
