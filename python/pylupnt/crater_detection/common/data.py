...
from .conics import MaskGenerator


class DataGenerator(MaskGenerator):
...
def make_dataset(
    ...
    if os.path.exists(output_path):
        raise ValueError(
            f"Dataset named `{os.path.basename(output_path)}` already exists!"
        )
    ...
    with h5py.File(output_path, "w") as hf:
        ...
        for group_name, dset_size in zip(
            ("training", "validation", "test"), (n_training, n_validation, n_testing)
        ):
            print(f"Creating dataset '{group_name}' @ {dset_size} images")
            group = hf.create_group(group_name)

            (images, masks, position, attitude, date, sol_incidence, A_craters) = (
                generate(dset_size, **generation_kwargs_)
            )
            for ds, name in zip(
...
def inspect_dataset(
    ...
    if plot:
        ...
        for i in range(n_inspect):
            ...
            weights = np.ones_like(images[i, 0].flatten()) / float(
                len(images[i, 0].flatten())
            )
            axes[i, 1].hist(
                images[i, 0].flatten(), n_bins, pixel_range, color="r", weights=weights
            )
