# Credits: https://github.com/wdoppenberg/crater-detection
...
from .robbins import load_craters, extract_robbins_dataset
...
def scale_det(matrix):
    ...
    if isinstance(matrix, np.ndarray):
        return np.cbrt((1.0 / LA.det(matrix)))[..., None, None] * matrix
...
def ellipse_axes(A):
    if isinstance(A, torch.Tensor):
        lambdas = (
            torch.linalg.eigvalsh(A[..., :2, :2])
            / (-torch.det(A) / torch.det(A[..., :2, :2]))[..., None]
        )
        axes = torch.sqrt(1 / lambdas)
    elif isinstance(A, np.ndarray):
        lambdas = (
            LA.eigvalsh(A[..., :2, :2])
            / (-LA.det(A) / LA.det(A[..., :2, :2]))[..., None]
        )
...
def plot_conics(
    A_craters: Union[np.ndarray, torch.Tensor],
...
def generate_mask(
    ...
    a_proj, b_proj, psi_proj, r_pix_proj = map(
        lambda i: np.round(i).astype(int), (a_proj, b_proj, psi_proj, r_pix_proj)
    )
    ...
    for i, (a, b, x, y, psi) in enumerate(
        zip(a_proj, b_proj, *r_pix_proj.T, psi_proj), 1
    ):
        mask = cv2.ellipse(
            mask, (x, y), (a, b), psi, 0, 360, i if instancing else 1, thickness
        )
...
def crater_camera_homography(r_craters, P_MC):
    ...
    H_Mi = np.concatenate(
        (np.concatenate(ENU_system(r_craters), axis=-1) @ S, r_craters), axis=-1
    )

    return P_MC @ np.concatenate(
...
def project_crater_centers(r_craters, fov, resolution, T_CM, r_M):
    ...
    P_MC = projection_matrix(K, T_CM, r_M)
    H_Ci = crater_camera_homography(r_craters, P_MC)
    return (H_Ci @ np.array([0, 0, 1]) / (H_Ci @ np.array([0, 0, 1]))[:, -1][:, None])[
        :, :2
    ]
...
class ConicProjector(Camera):
    ...
    def project_crater_centers(self, r_craters):
        H_Ci = crater_camera_homography(r_craters, self.projection_matrix)
        return (
            H_Ci @ np.array([0, 0, 1]) / (H_Ci @ np.array([0, 0, 1]))[:, -1][:, None]
        )[:, :2]
    ...
    def generate_mask(self, A_craters=None, C_craters=None, r_craters=None, **kwargs):
        ...
        if A_craters is None:
            if C_craters is None or r_craters is None:
                raise ValueError(
                    "Must provide either crater data in respective ENU-frame (C_craters & r_craters) "
...
class MaskGenerator(ConicProjector):
    ...
    def from_robbins_dataset(
        ...
        **load_crater_kwargs
        ...
        lat_cat, long_cat, major_cat, minor_cat, psi_cat, crater_id = (
    ...
    def _visible(self):
        return (
            cdist(self.r_craters_catalogue.squeeze(), self.position.T)
            <= np.sqrt(2 * self.height * self._primary_body_radius + self.height**2)
        ).ravel()
    ...
    def visible_catalogue_craters(self, margin=None):
        ...
        in_image = np.logical_and.reduce(
            np.logical_and(
                r_craters_img > -margin, r_craters_img < self.resolution[0] + margin
            ),
            axis=1,
        )
    ...
    def craters_in_image(self, margin=None):
        ...
        a_proj, b_proj = ellipse_axes(A_craters)
        axis_filter = np.logical_and(
            a_proj >= self.axis_threshold[0], b_proj >= self.axis_threshold[0]
        )
        axis_filter = np.logical_and(
            axis_filter,
            np.logical_and(
                a_proj <= self.axis_threshold[1], b_proj <= self.axis_threshold[1]
            ),
        )
    ...
    def generate_mask(self, **kwargs):
        ...
        return super(MaskGenerator, self).generate_mask(
            A_craters=self.craters_in_image(), **mask_args
        )

    def plot(self, *args, **kwargs):
        super(MaskGenerator, self).plot(
            A_craters=self.craters_in_image(), *args, **kwargs
        )
