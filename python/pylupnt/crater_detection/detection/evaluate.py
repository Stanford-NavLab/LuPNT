...
class Evaluator:
    def __init__(
        self, model=None, device="cpu", dataset_path="data/dataset_crater_detection.h5"
    ):
        if model is None:
        ...
        if isinstance(device, str):
            ...
            raise ValueError(
                "Type for device is incorrect, must be str or torch.device."
            )
    ...
    def make_grid(self, n_rows=3, n_cols=4, min_class_score=0.75):
        ...
        loader = DataLoader(
            self.ds, batch_size=1, shuffle=True, num_workers=0, collate_fn=collate_fn
        )
        ...
        for row in range(n_rows):
            for col in range(n_cols):
                ...
                matched_idxs, matched = get_matched_idxs(
                    pred[0]["boxes"][scores > min_class_score], targets[0]["boxes"]
                )
                ...
                plot_conics(
                    A_pred.cpu()[~matched], ax=axes[row, col], rim_color="yellow"
                )
    ...
    def performance_metrics(
        self, iou_threshold=0.5, confidence_thresholds=None, distance_threshold=None
    ):

        if confidence_thresholds is None:
            confidence_thresholds = torch.arange(start=0.05, end=0.99, step=0.05).to(
                self.device
            )

        loader = DataLoader(
            self.ds, batch_size=32, shuffle=True, num_workers=0, collate_fn=collate_fn
        )
        ...
        precision = torch.zeros(
            (len(loader), loader.batch_size, len(confidence_thresholds)),
            device=self.device,
        )
        recall = torch.zeros(
            (len(loader), loader.batch_size, len(confidence_thresholds)),
        ...
        for batch, (images, targets_all) in enumerate(bar):
            images = list(image.to(self.device) for image in images)
            targets_all = [
                {k: v.to(self.device) for k, v in t.items()} for t in targets_all
            ]
        ...
        for i in range(len(confidence_thresholds)):
            precision_out[i], recall_out[i], f1_out[i], iou_out[i], dist_out[i] = map(
                lambda x: x[..., i][x[..., i] > 0.0].mean(),
                (precision, recall, f1, iou, dist),
            )

        precision, recall, f1, iou, dist = map(
            lambda x: x.mean((0, 1)), (precision, recall, f1, iou, dist)
        )
    ...
    def precision_recall_plot(self, iou_thresholds=None, confidence_thresholds=None):
        ...
        for iou_threshold in iou_thresholds:
            precision, recall, f1, iou, dist, confidence_thresholds = (
                self.performance_metrics(iou_threshold=iou_threshold)
            )
