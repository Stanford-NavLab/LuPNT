...
from ..common.conics import conic_center, plot_conics
from ..common.data import inspect_dataset
    ...
    def __getitem__(self, idx: ...) -> Tuple[torch.Tensor, Dict]:
        ...
        with h5py.File(self.file_path, "r") as dataset:
            position = torch.as_tensor(
                dataset[self.group]["position"][idx], dtype=torch.float64
            )
            attitude = torch.as_tensor(
                dataset[self.group]["attitude"][idx], dtype=torch.float64
            )
        ...
        masks, obj_ids, boxes, area = map(
            lambda x: x[area_filter], (masks, obj_ids, boxes, area)
        )
...
class CraterEllipseDataset(CraterMaskDataset):
    ...
    def __getitem__(self, idx: ...) -> Tuple[torch.Tensor, Dict]:
        ...
        if len(x_box) > 0 and len(x) > 0:
            matched_idxs = cdist(
                np.vstack((x_box.numpy(), y_box.numpy())).T, np.vstack((x, y)).T
            ).argmin(1)
...
def get_dataloaders(
    dataset_path: str, batch_size: int = 10, num_workers: int = 2
    ...
    validation_dataset = CraterEllipseDataset(
        file_path=dataset_path, group="validation"
    )
    ...
    test_loader = DataLoader(
        test_dataset, batch_size=1, num_workers=0, collate_fn=collate_fn, shuffle=True
    )
...
def train_model(
    ...
    if pretrained:
        ...
        optimizer = SGD(
            params, lr=initial_lr, momentum=momentum, weight_decay=weight_decay
        )
    ...
    with mlflow.start_run(**run_args) as run:
        ...
        if not pretrained:
            ...
            for tp in tracked_params:
                try:
                    ...
                    pass
            mlflow.log_figure(
                inspect_dataset(dataset_path, return_fig=True, summary=False),
                f"dataset_inspection.png",
            )
        ...
        for e in range(start_e, num_epochs + start_e):
            ...
            mlflow.log_metric(
                "lr", optimizer.state_dict()["param_groups"][0]["lr"], step=e
            )
            ...
            for k, v in run_metrics["train"].items():
                ...
                mlflow.log_metric(
                    "train_" + k,
                    mean(v[(e - 1) * len(train_loader) : e * len(train_loader)]),
                    step=e,
                )
            ...
            for k, v in run_metrics["valid"].items():
                ...
                mlflow.log_metric(
                    "valid_" + k,
                    mean(
                        v[(e - 1) * len(validation_loader) : e * len(validation_loader)]
                    ),
                    step=e,
                )
            scheduler.step(
                mean(
                    run_metrics["valid"]["loss_total"][
                        (e - 1) * len(validation_loader) : e * len(validation_loader)
                    ]
                )
            )
            ...
            m1, m2 = map(
                lambda arr: torch.vstack(tuple(conic_center(arr).T)).T[..., None],
