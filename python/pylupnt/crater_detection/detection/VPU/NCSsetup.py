...
class OpenVINOHandler:
    ...
    def __init__(self, model_name, device="MYRIAD", root="VPU/IR/"):
        ...
        log.info(
            "\tinput shape: "
            + str(self.net.input_info[self.input_key].input_data.shape)
        )
    ...
    def infer(self, batch):
        ...
        if batch.shape[1:-1] != self.input_shape[1:-1]:
            raise ValueError(
                f"Batch shape does not match input! Expected {self.input_shape}, received {batch.shape}."
            )
    ...
    def info(self):
        ...
        return (
            f"""{self.__class__.__name__}({self.device}):\n"""
