...
def draw_patches(
    ...
    img, bboxes, labels, scores, masks = map(
    ...
    for (xmin, ymin, xmax, ymax), s in zip(bboxes, scores):
        ...
        patch = patches.Rectangle(
            (cx - 0.5 * w, cy - 0.5 * h), w, h, fill=False, color="r"
        )
        ax.add_patch(patch)
        ...
        if w < 15:
            ...
            ax.text(
                cx, cy, f"{s:.0%}", ha="center", va="center", size=8, bbox=bbox_props
            )
...
def draw_detections(df, shape=(256, 256)):
    ...
    for i, r in df.iterrows():
        ...
        img_ellipses = cv2.circle(
            img_ellipses, center_coordinates, 0, (255, 255, 255), 1
        )
