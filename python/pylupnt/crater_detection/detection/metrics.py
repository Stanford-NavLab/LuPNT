...
from ..common.conics import conic_center, scale_det, ellipse_axes


def get_matched_idxs(
    pred: Union[Dict, torch.Tensor],
    ...
    if len(boxes_pred) > 0 and len(boxes_target) > 0:
        ...
        if return_iou:
            return (
                matched_idxs,
                iou_list > iou_threshold,
                iou_list[iou_list > iou_threshold],
            )
        else:
        ...
        if return_iou:
            return (
                torch.zeros(0).to(pred),
                torch.zeros(0, dtype=torch.bool).to(pred),
                torch.zeros(0).to(pred),
            )
        else:
...
def detection_metrics(
    ...
    if len(boxes_pred_conf) > 0 and len(boxes_target) > 0:
        ...
        matched_idxs, matched, iou_list = get_matched_idxs(
            boxes_pred_conf, boxes_target, iou_threshold=iou_threshold, return_iou=True
        )

        if len(A_target[matched_idxs][matched]) > 0:
            dist = (
                gaussian_angle_distance(A_pred_conf, A_target[matched_idxs])
                .mean()
                .item()
            )
        ...
        matched_idxs_fn, matched_FN = get_matched_idxs(
            boxes_pred[scores < confidence_threshold],
            boxes_target,
            ...
            if len(matched_idxs_fn) > 0 and matched_FN.sum() > 0:
                matched_FN = matched_FN & (
                    gaussian_angle_distance(
                        A_pred[scores < confidence_threshold], A_target[matched_idxs_fn]
                    )
                    <= distance_threshold
                )
            else:
...
def mv_kullback_leibler_divergence(
    ...
    m1, m2 = map(
        lambda arr: torch.vstack(tuple(conic_center(arr).T)).T[..., None], (A1, A2)
    )
...
def gaussian_angle_distance(
    ...
    if isinstance(cov1, torch.Tensor) and isinstance(cov2, torch.Tensor):
        ...
        m1, m2 = map(
            lambda arr: np.vstack(tuple(conic_center(arr).T)).T[..., None], (A1, A2)
        )
