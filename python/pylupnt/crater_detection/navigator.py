...
class LunarNavigator(CraterDetector, Camera):
    def __init__(self, database: CraterDatabase, ekf: EKF, **kwargs):
    ...
    @torch.no_grad()
    def derive_position(
        self, image: Union[np.ndarray, torch.Tensor], attitude, confidence=0.75
    ):
        self.eval()
        ...
        return self.database.query_position(
            A_detections=A_detections, T=attitude, K=self.camera_matrix
        )
