...
MOON_SURFACE = Image.open(os.path.join(LUPNT_DATA_PATH, "topo", "moon_surface.jpeg"))
EARTH_SURFACE = Image.open(os.path.join(LUPNT_DATA_PATH, "topo", "earth_surface.jpg"))
IMAGES = {
    _pnt.MOON: np.asarray(MOON_SURFACE),
    _pnt.EARTH: np.asarray(EARTH_SURFACE),
}
RADII = {
    _pnt.MOON: _pnt.R_MOON,
...
def plot_frame(
    ...
    plot_arrow3(fig, origin, R_w2b[:, 0], length[0], tip, "red", width, scale)
    plot_arrow3(fig, origin, R_w2b[:, 1], length[1], tip, "green", width, scale)
    plot_arrow3(fig, origin, R_w2b[:, 2], length[2], tip, "blue", width, scale)
    ...
    for i in range(N_sat):
        fig.add_scatter3d(
            **dict(x=rv[i, :, 0], y=rv[i, :, 1], z=rv[i, :, 2]),
            mode="lines",
            line=dict(color=color[i % len(color)] if type(color) == list else color, width=3),
...
def image2zvals(
    ...
    model = KMeans(n_clusters=n_colors, random_state=rngs, n_init="auto").fit(training_pixels)
    ...
    pl_colorscale = [[sv, f"rgb{tuple(int(c) for c in color)}"] for sv, color in zip(scale, colors)]
...
def mesh_data(img, n_colors=32, n_training_pixels=800):
    ...
    tri_color_intensity = np.array([zc[k][2] if k % 2 else zc[k][1] for k in range(len(zc))])
...
def plot_antenna_gain_pattern(
    ...
    fig.update_layout(scene=dict(xaxis=tmp, yaxis=tmp, zaxis=tmp), width=600, height=400)
    return fig
