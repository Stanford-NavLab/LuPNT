...
_output_path_str = os.getenv("LUPNT_OUTPUT_PATH")
LUPNT_OUTPUT_PATH = Path(_output_path_str) if _output_path_str else None
...
def get_output_dir(output_dirs=None):
    if LUPNT_OUTPUT_PATH is None:
        output_dir = os.path.join(LUPNT_DATA_PATH, "output")
    else:
        output_dir = LUPNT_OUTPUT_PATH
    if output_dirs is not None:
        output_dir = os.path.join(output_dir, output_dirs)
...
def load_data(directory):
    ...
    for filename in os.listdir(output_path):
        ...
        if ext == ".csv":
            try:
                data[name] = np.loadtxt(os.path.join(output_path, filename), delimiter=",")
            except ValueError:
...
def plot_RTN(rv_RTN, labels=None, legend_text=None, init=False, final=True, center=True):
    ...
    gs = GridSpec(2, 2, height_ratios=[1, 1], width_ratios=[wr, 1], hspace=0.25, wspace=0.25)
...
def print_aligned(matrix):
    ...
    for r, row in enumerate(matrix):
        ...
        for i, value in enumerate(row):
            ...
            before, after = str(value).split(".") if "." in str(value) else (str(value), "")
