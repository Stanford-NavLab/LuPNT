...
img = np.asarray(
    Image.open(os.path.join(utils.LUPNT_DATA_PATH, "topo", "moon_surface.jpeg"))
)
img = img[::5, ::5]
RE = 1737.1
...
def create_sphere_meshgrid(
    ...
    """
    u, v = np.meshgrid(np.linspace(0, 2 * np.pi, cols), np.linspace(0, np.pi, rows))
    return RE * np.cos(u) * np.sin(v), RE * np.sin(u) * np.sin(v), RE * np.cos(v)


def get_moon_trace() -> go.Mesh3d:
    """
    Get the moon trace
    ...
    """
    r, c, _ = img.shape
    x, y, z = create_sphere_meshgrid(r, c)
    I, J, K, tri_color_intensity, pl_colorscale = mesh_data(
        img, n_colors=32, n_training_pixels=10000
    )
    return go.Mesh3d(
        x=x.flatten(),
        y=y.flatten(),
