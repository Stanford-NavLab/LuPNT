...
struct LinkParams {
  ...
  // Agent Parameters
  std::shared_ptr<Body>
      tx_center_body;  // center body of the transmitter (target)
  std::shared_ptr<Body> rx_center_body;  // center body of the receiver
...
class LinkMeasurement {
 private:
  ...
  // state size
  int state_size_ow_ =
      16;  // One way link state size (target rv + clock,  receiver rv + clock)
 ...
 public:
  ...
  inline void SetFixedRangeError(double range_sigma) {
    range_sigma_fixed_ = range_sigma;
  }
  ...
  VecX GetTrueOneWayLinkMeasurement(
      std::vector<LinkMeasurementType> meas_types);

  VecX GetOneWayLinkMeasurement(Real epoch_rx, Vec6 rv_tx, Vec6 rv_rx,
  ...
  VecX GetTrueTwoWayLinkMeasurement(
      std::vector<LinkMeasurementType> meas_types);

  VecX GetTwoWayLinkMeasurement(Real epoch_rx, Vec6 rv_tx, Vec6 rv_rx,
                                MatXd H_tw_rx, Real hardware_delay,
                                std::vector<LinkMeasurementType> meas_types,
                                bool with_noise, bool with_jacobian);

  Real GetTwoWayRangeMeasurement(Real epoch_rx, Vec6 rv_receiver,
                                 Vec6 rv_target, MatXd &H_tw_range,
                                 Real hardware_delay, bool with_noise,
                                 bool with_jacobian);

  Real GetTwoWayRangeRateMeasurement(Real epoch_rx, Vec6 rv_receiver,
