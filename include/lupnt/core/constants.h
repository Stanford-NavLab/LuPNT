...
static std::filesystem::path GetCspiceKernelDir() {
  return GetDataPath() / "ephemeris";
}
static std::filesystem::path GetAsciiKernelDir() {
  return GetDataPath() / "ephemeris" / "ascii";
}
...
static double BodyRadius(NaifId id) {
  switch (id) {
    case NaifId::SUN:
      return 696342.0;
    case NaifId::MERCURY:
