...
namespace lupnt {
  ...
  const std::map<Frame, std::string_view> frame2string = {
  ...
  const std::map<Time, std::string_view> time2string = {
  ...
  namespace spice {
    ...
    Real String2TDB(std::string_view str);
    Real String2TAI(std::string_view str);

    std::string TAItoStringUTC(Real t_tai, int prec);
    std::string TDBtoStringUTC(Real t_tdb, int prec);

    Real ConvertTime(Real t, std::string_view from_time, std::string_view to_time);
    ...
    Vec3d GetBodyPosSpice(Real t_tai, NaifId obs, NaifId target, Frame refFrame = Frame::GCRF,
                          std::string_view abCorrection = "NONE");
