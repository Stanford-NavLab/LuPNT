# Registers a Jupyter kernel named "lupnt" for the active Python interpreter so that
# VSCode (and any other Jupyter front-end) can find the pixi environment in the kernel
# picker without manual setup.
#
# Runs once at CMake configure time; skips silently if the kernel spec already exists.
# Requires PYTHON_EXECUTABLE to be set (done by CondaAware.cmake when pixi/conda is
# active). Disable with -DLUPNT_REGISTER_KERNEL=OFF.
...
if(NOT EXISTS "${_KERNEL_JSON}")
  ...
  execute_process(
    COMMAND
      "${PYTHON_EXECUTABLE}" -m ipykernel install --user --name lupnt --display-name
      "lupnt (pixi)"
...
# regardless of how it's launched, so patch it here to mirror pixi's [activation] env. Re-applied
# on every configure (cheap) in case pixi.toml's activation env changes.
if(EXISTS "${_KERNEL_JSON}")
  ...
  message(STATUS "JupyterKernel: kernel 'lupnt' env configured (PYTHONPATH=${LUPNT_REPO_ROOT}/python)")
endif()
