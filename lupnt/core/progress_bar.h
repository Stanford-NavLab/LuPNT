...
class ProgressBar {
 public:
  ...
  void Update(int value) {
    ...
    int newProgress =
        static_cast<int>(static_cast<double>(value) / total_ * 100);
    if (newProgress > currentProgress_ &&
        elapsedSinceLastUpdate.count() >= maxUpdateFrequency_) {
      double itersPerSecond = value / elapsedSinceStart.count();
      int remainingTime =
          itersPerSecond > 0
              ? static_cast<int>((total_ - value) / itersPerSecond)
              : 0;
  ...
  void Done() {
    ...
    std::cout << "Elapsed time: "
              << FormatTime(static_cast<int>(elapsed.count())) << std::endl;
  }
