...
class Scheduler {
 ...
 public:
  ...
  static void ScheduleFunction(
      std::function<void(double)> func, double time = 0.0,
      double freq = std::numeric_limits<double>::infinity()) {
    ScheduleEvent(Event(time, func, freq));
  }

  static void ScheduleApplication(
      Application &app, double time = 0.0,
      double freq = std::numeric_limits<double>::infinity()) {
    app.Setup();
    ScheduleEvent(
        Event(time, [&app](double t) { app.Step(t); }, app.GetFrequency()));
  }
