...
VectorX Occultation::ComputeOccultation(real epoch, const Vector3& r1,
  ...
  for (size_t i = 0; i < bodies.size(); i++) {
    ...
    Vector3 rb = SpiceInterface::GetBodyPosVel(
                     epoch, NaifId::SOLAR_SYSTEM_BARYCENTER, bodies[i])
                     .head(3);
    ...
    if (r1b.norm() <= r2b.norm()) {
      if (r1b.dot(r12) <= 0) {
        ...
        Vector3 r12_unit = r12.normalized();
        Vector3 rblos = r1b.dot(r12_unit) * r12_unit - r1b;  // body->los
      ...
      if (r2b.dot(-r12) <= 0) {
        ...
        // Compute distance to line of sight
        Vector3 r12_unit = -r12.normalized();
        Vector3 rblos = r2b.dot(r12_unit) * r12_unit - r2b;  // body->los
        distances(i) = rblos.norm();
      }
    }
  }
...
MatrixX Occultation::ComputeOccultation(real epoch, const Matrix<-1, 3>& r1,
  ...
  assert((r1.rows() == r2.rows() || r1.rows() == 1 || r2.rows() == 1) &&
         ...
         "only one row");
  MatrixX distances(std::max(r1.rows(), r2.rows()), bodies.size());
  for (int i = 0; i < std::max(r1.rows(), r2.rows()); i++) {
    Vector3 r1_vec = r1.rows() == 1 ? r1.row(0) : r1.row(i);
    Vector3 r2_vec = r2.rows() == 1 ? r2.row(0) : r2.row(i);
    distances.row(i) =
        ComputeOccultation(epoch, r1_vec, r2_vec, cs1, cs2, bodies);
  }
  return distances;
}
...
MatrixX Occultation::ComputeOccultation(const VectorX& epoch,
  ...
  MatrixX distances(epoch.size(), bodies.size());
  for (int i = 0; i < epoch.size(); i++) {
    Vector3 r1_vec = r1.rows() == 1 ? r1.row(0) : r1.row(i);
    Vector3 r2_vec = r2.rows() == 1 ? r2.row(0) : r2.row(i);
    distances.row(i) =
        ComputeOccultation(epoch(i), r1_vec, r2_vec, cs1, cs2, bodies);
  }
  return distances;
}
