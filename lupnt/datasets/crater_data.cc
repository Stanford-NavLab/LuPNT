...
std::vector<Crater> CraterDataLoader::LoadCraters(
    const std::string& filename, const std::pair<double, double>* latlims,
    const std::pair<double, double>* longlims,
    const std::pair<double, double>& diamlims, const double ellipse_limit,
  ...
  for (const auto& row : data) {
    ...
    try {
      ...
      if (longlims && (lon < longlims->first || lon > longlims->second))
        continue;
      ...
      craters.push_back(
          {lat, lon, diam_major, diam_minor, std::stod(row.at(6)), row.at(0)});
...
void CraterDataLoader::ExtractRobbinsDataset(
    const std::vector<Crater>& craters, Eigen::VectorXd& lat,
    Eigen::VectorXd& lon, Eigen::VectorXd& major, Eigen::VectorXd& minor,
    Eigen::VectorXd& psi, std::vector<std::string>& crater_id, bool radians) {
