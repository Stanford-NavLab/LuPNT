...
void LoadTaiUtcFileData(const std::filesystem::path& filepath) {
  ...
  while (std::getline(file, line)) {
    ...
    std::string year, month, day, eq_jd, tai_utc_eq, s1, plus, paren_mjd, minus,
        paren, times, s2;

    iss >> year >> month >> day >> eq_jd >> jd >> tai_utc_eq >> tai_utc >> s1 >>
        plus >> paren_mjd >> minus >> mjd0 >> paren >> times >> scale >> s2;
...
double Evaluate(double mjd, int i) {
  return tai_utc_data->tai_utc(i) +
         tai_utc_data->scale(i) * (mjd - tai_utc_data->mjd0(i));
}
