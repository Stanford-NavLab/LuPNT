 ...
 */
//------------------------------------------------------------------------------
Interpolator::~Interpolator() { CleanupArrays(); }
...
//------------------------------------------------------------------------------
//  bool AddPoint(double ind, double *data)
//------------------------------------------------------------------------------
/**
 ...
 * interpolation routine (Interpolate(const double, double*)) to generate the
 ...
 */
//------------------------------------------------------------------------------
bool Interpolator::AddPoint(double ind, double *data) {
  int i;

  if (!independent) AllocateArrays();

  // Reset the index into the array if at the end
  if (latestPoint == bufferSize - 1) latestPoint = -1;

  dataIncreases = (ind > previousX ? true : false);
  previousX = ind;

  independent[++latestPoint] = ind;
  for (i = 0; i < dimension; ++i) dependent[latestPoint][i] = data[i];

  ++pointCount;
  rangeCalculated = false;

  //    MessageInterface::ShowMessage
  //       ("===> Interpolator::AddPoint() ind=%f, data[0]=%f added,
  //       latestPoint=%d, "
...
//------------------------------------------------------------------------------
//  int IsInterpolationFeasible(double ind)
//------------------------------------------------------------------------------
/**
 ...
 */
//------------------------------------------------------------------------------
int Interpolator::IsInterpolationFeasible(double ind) { return 1; }
 ...
 */
//------------------------------------------------------------------------------
bool Interpolator::GetForceInterpolation() { return forceInterpolation; }
 ...
 */
//------------------------------------------------------------------------------
void Interpolator::SetExtrapolation(bool flag) { allowExtrapolation = flag; }
 ...
 */
//------------------------------------------------------------------------------
int Interpolator::GetBufferSize() { return bufferSize; }
 ...
 */
//------------------------------------------------------------------------------
int Interpolator::GetPointCount() { return pointCount; }
 ...
 */
//------------------------------------------------------------------------------
std::string Interpolator::GetName() { return instanceName; }
 ...
 */
//------------------------------------------------------------------------------
bool Interpolator::GetRange(double &lower, double &upper) {
  if (!rangeCalculated) SetRange();
...
void Interpolator::AllocateArrays() {
  ...
  independent = new double[bufferSize];
  dependent = new double *[bufferSize];
  for (i = 0; i < bufferSize; ++i) dependent[i] = new double[dimension];
  latestPoint = -1;
}
...
void Interpolator::CleanupArrays() {
  ...
  if (independent) delete[] independent;

  if (dependent) {
    for (i = 0; i < bufferSize; ++i) delete[] dependent[i];
    delete[] dependent;
  }
...
void Interpolator::CopyArrays(const Interpolator &i) {
  ...
  AllocateArrays();
  memcpy(independent, i.independent, bufferSize * sizeof(double));
  for (j = 0; j < bufferSize; ++j)
    memcpy(dependent[j], i.dependent[j], dimension * sizeof(double));
}
...
void Interpolator::SetRange() {
  if (rangeCalculated) return;

  // Not enough points -- just continue
  if (requiredPoints > pointCount) return;

  range[0] = range[1] = independent[0];

  int i, max = (pointCount > bufferSize ? bufferSize : pointCount);

  for (i = 1; i < max; ++i) {
    if (independent[i] < range[0]) range[0] = independent[i];
    if (independent[i] > range[1]) range[1] = independent[i];
  }
