...
double LinearInterp2d(const VecXd& x, const VecXd& y, const VecXd& data,
                      double xi, double yi) {
  ...
  double result = data(i0, j0) * dx1 * dy1 + data(i0, j1) * dx1 * dy0 +
                  data(i1, j0) * dx0 * dy1 + data(i1, j1) * dx0 * dy0;
  return result;
}

LagrangeInterpolator::LagrangeInterpolator(const VecXd& x, double xi,
                                           uint order)
