...
CartesianOrbitState FrameConverter::Convert(real t_tai,
  ...
  Vec6 rv_out = Convert(t_tai, rv_in, state_in.GetCoordSystem(), frame_out);
  return CartesianOrbitState(rv_out, frame_out);
}

Vec3 FrameConverter::Convert(real t_tai, Vec3 rv_in, Frame frame_in,
                             Frame frame_out) {
  Vec6 rv_in_6;
  rv_in_6 << rv_in, Vec3::Zero();
  Vec6 rv_out_6 = Convert(t_tai, rv_in_6, frame_in, frame_out);
  return rv_out_6.head(3);
}

Mat<-1, 6> FrameConverter::Convert(real t_tai, const Mat<-1, 6> &rv_in,
...
Mat<-1, 3> FrameConverter::Convert(real t_tai, const Mat<-1, 3> &r_in,
                                   Frame frame_in, Frame frame_out) {
  Mat<-1, 6> rv_in(r_in.rows(), 6);
  rv_in << r_in, Mat<-1, 3>::Zero(r_in.rows(), 3);
  Mat<-1, 6> rv_out = Convert(t_tai, rv_in, frame_in, frame_out);
  return rv_out.leftCols(3);
}

Mat<-1, 6> FrameConverter::Convert(VecX t_tai, const Vec6 &rv_in,
                                   Frame frame_in, Frame frame_out) {
...
Mat<-1, 3> FrameConverter::Convert(VecX t_tai, const Vec3 &r_in, Frame frame_in,
                                   Frame frame_out) {
...
Mat<-1, 6> FrameConverter::Convert(VecX t_tai, const Mat<-1, 6> &rv_in,
                                   Frame frame_in, Frame frame_out) {
...
Mat<-1, 3> FrameConverter::Convert(VecX t_tai, const Mat<-1, 3> &r_in,
                                   Frame frame_in, Frame frame_out) {
 ...
 */
Vec6 FrameConverter::Convert(real t_tai, Vec6 rv_in, Frame frame_in,
                             Frame frame_out) {
  ...
  switch (frame_in) {
    case ITRF: {
      switch (frame_out) {
        case GCRF: {
          // Convert to GCRF
          Mat6 Rrv_itrf2gcrf =
              GetFrameConversionMat(t_tai, Frame::ITRF, Frame::GCRF);
          Vec6 rv_gcrf = Rrv_itrf2gcrf * rv_in;
          return rv_gcrf;
        }
        default: {  // First convert to GCRF and then to the desired frame
          Vec6 rv_gcrf = Convert(t_tai, rv_in, ITRF, GCRF);
          Vec6 rv_out = Convert(t_tai, rv_gcrf, GCRF, frame_out);
          return rv_out;
        }
      }
    }
    ...
    case MOON_ME: {
      switch (frame_out) {
        case MOON_PA: {  // Convert to MOON_PA
          ...
          Mat3 B_M = Rot1(-0.2785 * DEG_PER_ARCSEC) *
        ...
        default: {  // first convert to MOON_PA and then to the desired frame
          Vec6 rv_pa = Convert(t_tai, rv_in, MOON_ME, MOON_PA);
          Vec6 rv_out = Convert(t_tai, rv_pa, MOON_PA, frame_out);
          return rv_out;
        }
      }
    }
    ...
    case MOON_PA: {
      switch (frame_out) {
        ...
        default: {  // first convert to MOON_CI and then to the desired frame
          Vec6 rv_mi = Convert(t_tai, rv_in, MOON_PA, MOON_CI);
          Vec6 rv_out = Convert(t_tai, rv_mi, MOON_CI, frame_out);
          return rv_out;
        }
      }
    }
    ...
    case GCRF: {
      switch (frame_out) {
        ...
        case ITRF: {
          Mat6 Rrv_gcrf2itrf =
              GetFrameConversionMat(t_tai, Frame::GCRF, Frame::ITRF);
        ...
        case EMR: {
          ...
          Vec6 rv_emr = InertialToRtn(rv_icrf_emb2e, rv_in);
          return rv_emr;
        }
        case MOON_PA:
        case MOON_ME: {
          Vec6 rv_mi = Convert(t_tai, rv_in, GCRF, MOON_CI);
          Vec6 rv_out = Convert(t_tai, rv_mi, MOON_CI, frame_out);
          return rv_out;
        }
        default:
    ...
    case MOON_CI: {
      switch (frame_out) {
        ...
        case MOON_ME: {
          Vec6 rv_pa = Convert(t_tai, rv_in, MOON_CI, MOON_PA);
          Vec6 rv_out = Convert(t_tai, rv_pa, MOON_PA, frame_out);
          return rv_out;
        }
        case MOON_OP: {
          Mat6 R_op2mi = ComputeOpToMi(t_tai);
          Vec6 rv_op = R_op2mi.transpose() * rv_in;
          return rv_op;
        }
        default: {
          Vec6 rv_gcrf = Convert(t_tai, rv_in, MOON_CI, GCRF);
          Vec6 rv_out = Convert(t_tai, rv_gcrf, GCRF, frame_out);
          return rv_out;
        }
      }
    ...
    case ICRF: {
      switch (frame_out) {
        ...
        default: {
          Vec6 rv_gcrf = Convert(t_tai, rv_in, ICRF, GCRF);
          Vec6 rv_out = Convert(t_tai, rv_gcrf, GCRF, frame_out);
          return rv_out;
        }
      }
    ...
    case EMR: {
      switch (frame_out) {
        case GCRF: {
          ...
          Vec6 rv_gcrf = RtnToInertial(rv_icrf_emb2e, rv_in);
          return rv_gcrf;
        }
        default: {
          Vec6 rv_gcrf = Convert(t_tai, rv_in, EMR, GCRF);
          Vec6 rv_out = Convert(t_tai, rv_gcrf, GCRF, frame_out);
          return rv_out;
        }
      }
    ...
    case MOON_OP: {
      switch (frame_out) {
        ...
        default: {
          Vec6 rv_mi = Convert(t_tai, rv_in, MOON_OP, MOON_CI);
          Vec6 rv_out = Convert(t_tai, rv_mi, MOON_CI, frame_out);
          return rv_out;
        }
      }
...
Mat6 FrameConverter::ComputeOpToMi(real t_tai) {
  Vec6 rv_earth_icrf =
  ...
  // Moon axis
  MatX iau_moon2icrf;
  iau_moon2icrf = GetFrameConversionMat(t_tai, Frame::MOON_PA, Frame::GCRF)
                      .block(0, 0, 3, 3);
