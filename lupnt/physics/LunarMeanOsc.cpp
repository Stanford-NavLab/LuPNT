...
ad::Vector6real MeanToOsculating(ad::Vector6real meanCoeVec) {
  double t = 0.0;  // Time [s]
  double nM = 2.66e-6;
  ad::Vector6real meanDoeVec = CoeToDelaunay(meanCoeVec, MU_MOON, nM, t);
  ...
  std::array<double, 6> sp2 =
      ComputeSecondOrderShortPeriod(meanCoeVec, meanDoeVec);
  std::array<double, 6> mp1 =
      ComputeFirstOrderMediumPeriod(meanCoeVec, meanDoeVec);
  std::array<double, 6> mp2 =
      ComputeSecondOrderMediumPeriod(meanCoeVec, meanDoeVec);

  double l =
      lpp + sp2[0] + mp1[0] + mp2[0];  // l = lpp + lpp_sp2 + lpp_mp1 + lpp_mp2;
  double g =
      gpp + sp2[1] + mp1[1] + mp2[1];  // g = gpp + gpp_sp2 + gpp_mp1 + gpp_mp2;
  double h =
      hpp + sp2[2] + mp1[2] + mp2[2];  // h = hpp + hpp_sp2 + hpp_mp1 + hpp_mp2;
  double L =
      Lpp + sp2[3] + mp1[3] + mp2[3];  // L = Lpp + Lpp_sp2 + Lpp_mp1 + Lpp_mp2;
  double G =
      Gpp + sp2[4] + mp1[4] + mp2[4];  // G = Gpp + Gpp_sp2 + Gpp_mp1 + Gpp_mp2;
  double H =
      Hpp + sp2[5] + mp1[5] + mp2[5];  // H = Hpp + Hpp_sp2 + Hpp_mp1 + Hpp_mp2;

  ad::Vector6real oscDoeVec;
  oscDoeVec << l, g, h, L, G, H;

  ad::Vector6real oscCoeVec = DelaunayToCoe(oscDoeVec, MU_MOON, nM, t);
  return oscCoeVec;
}

ad::Vector6real OsculatingToMean(ad::Vector6real oscCoeVec) {
  double t = 0.0;
  double nM = 2.66e-6;
  ad::Vector6real oscDoeVec = CoeToDelaunay(oscCoeVec, MU_MOON, nM, t);
  ...
  std::array<double, 6> sp2 =
      ComputeSecondOrderShortPeriod(oscDoeVec, oscDoeVec);
  std::array<double, 6> mp1 =
      ComputeFirstOrderMediumPeriod(oscDoeVec, oscDoeVec);
  std::array<double, 6> mp2 =
      ComputeSecondOrderMediumPeriod(oscDoeVec, oscDoeVec);
  std::array<double, 6> mp1c =
      ComputeCorrectionMediumPeriod(oscCoeVec, oscDoeVec);

  double lpp = l - sp2[0] - mp1[0] - mp1c[0] -
               mp2[0];  // lpp = l - l_sp2 - l_mp1 - l_mp1c - l_mp2;
  double gpp = g - sp2[1] - mp1[1] - mp1c[1] -
               mp2[1];  // gpp = g - g_sp2 - g_mp1 - g_mp1c - g_mp2;
  double hpp = h - sp2[2] - mp1[2] - mp1c[2] -
               mp2[2];  // hpp = h - h_sp2 - h_mp1 - h_mp1c - h_mp2;
  double Lpp = L - sp2[3] - mp1[3] - mp1c[3] -
               mp2[3];  // Lpp = L - L_sp2 - L_mp1 - L_mp1c - L_mp2;
  double Gpp = G - sp2[4] - mp1[4] - mp1c[4] -
               mp2[4];  // Gpp = G - G_sp2 - G_mp1 - G_mp1c - G_mp2;
  double Hpp = H - sp2[5] - mp1[5] - mp1c[5] -
               mp2[5];  // Hpp = H - H_sp2 - H_mp1 - H_mp1c - H_mp2;

  ad::Vector6real meanDoeVec;
  meanDoeVec << lpp, gpp, hpp, Lpp, Gpp, Hpp;

  ad::Vector6real meanCoeVec = DelaunayToCoe(meanDoeVec, MU_MOON, nM, t);
  return meanCoeVec;
}
...
std::array<double, 6> ComputeSecondOrderShortPeriod(ad::Vector6real &coe,
  ...
  double B_2_2 =
      3 * e * sin(f + 2 * g) + 3 * sin(2 * f + 2 * g) + e * sin(3 * f + 2 * g);
  double B_22_1 =
      2 * (f - l) * cos(2 * h) + e * sin(f - 2 * h) + e * sin(f + 2 * h);
  double B_22_2 = 3 * e * sin(f + 2 * g - 2 * h) +
                  3 * sin(2 * f + 2 * g - 2 * h) +
                  e * sin(3 * f + 2 * g - 2 * h);
  double B_22_3 = 3 * e * sin(f + 2 * g + 2 * h) +
                  3 * sin(2 * f + 2 * g + 2 * h) +
                  e * sin(3 * f + 2 * g + 2 * h);
  double B_M_1 = 9 * e * (4 + pow(e, 2)) * sin(E) - 9 * pow(e, 2) * sin(2 * E) +
                 pow(e, 3) * sin(3 * E);
  double B_M_2 = 2 * cos(2 * h) *
                     (-9 * e * (4 + pow(e, 2)) * sin(E) +
                      9 * pow(e, 2) * sin(2 * E) - pow(e, 3) * sin(3 * E)) +
                 2 * cos(2 * g) *
                     (-15 * e * (2 + pow(e, 2)) * sin(E) +
                      3 * (2 + pow(e, 2)) * sin(2 * E) -
                      e * (2 - pow(e, 2)) * sin(3 * E)) +
                 2 * sin(2 * g) *
  ...
  double B_M_3 =
      ...
      2 * cos(2 * g - 2 * h) *
          (-15 * e * (2 + pow(e, 2)) * sin(E) +
           3 * (2 + pow(e, 2)) * sin(2 * E) - e * (2 - pow(e, 2)) * sin(3 * E));
  double B_M_4 =
      2 * sin(2 * g + 2 * h) *
          (-30 * e * sqrt(1 - pow(e, 2)) * cos(E) +
           6 * sqrt(1 - pow(e, 2)) * (1 + pow(e, 2)) * cos(2 * E) -
  ...
  double B_M_e_0 =
      6 * sin(E) *
      (6 + 5 * pow(e, 2) - 6 * e * cos(E) + pow(e, 2) * cos(2 * E));
  double B_M_e_1 =
      -12 * sin(E) *
          (6 + 5 * pow(e, 2) - 6 * e * cos(E) + pow(e, 2) * cos(2 * E)) *
          cos(2 * h) +
      4 * sin(E) *
          ((3 * pow(e, 2) - 2) * cos(2 * E) - 21 * pow(e, 2) + 6 * e * cos(E) -
           16) *
          cos(2 * g) +
      (4 / sqrt(1 - pow(e, 2))) *
          (15 * (2 * pow(e, 2) - 1) * cos(E) +
           3 * e * (1 - 3 * pow(e, 2)) * cos(2 * E) +
           (2 * pow(e, 2) - 1) * cos(3 * E)) *
          sin(2 * g);
  double B_M_e_2 = 4 * sin(E) *
                       ((3 * pow(e, 2) - 2) * cos(2 * E) - 21 * pow(e, 2) +
                        6 * e * cos(E) - 16) *
                       cos(2 * g - 2 * h) +
                   (4 / sqrt(1 - pow(e, 2))) *
                       (15 * (2 * pow(e, 2) - 1) * cos(E) +
  ...
  double B_M_E_0 =
      3 * e *
      (3 * (pow(e, 2) + 4) * cos(E) + e * (e * cos(3 * E) - 6 * cos(2 * E)));
  double B_M_E_1 = -6 * e *
                       (3 * (pow(e, 2) + 4) * cos(E) +
                        e * (e * cos(3 * E) - 6 * cos(2 * E))) *
                       cos(2 * h) +
                   6 *
                       (e * (pow(e, 2) - 2) * cos(3 * E) +
  ...
  double lsp2 =
      (-(J2_MOON * pow(R_MOON, 2)) /
       (32 * pow(a, 2) * e * pow(1 - pow(e, 2), 3 / 2.0))) *
          (-4 * (1 - 3 * pow(cos(i), 2)) *
               (pow(e, 2) * cos(2 * f) - pow(e, 2) + 6 * e * cos(f) + 6) *
               sin(f) +
           pow(sin(i), 2) * (-18 * e * sin(2 * g) -
                             3 * (4 + 5 * pow(e, 2)) * sin(f + 2 * g) +
                             3 * pow(e, 2) * sin(f - 2 * g) +
                             (28 - pow(e, 2)) * sin(3 * f + 2 * g) +
                             18 * e * sin(4 * f + 2 * g) +
                             3 * pow(e, 2) * sin(5 * f + 2 * g))) +
      (-(C22_MOON * pow(R_MOON, 2)) /
       (32 * pow(a, 2) * e * pow(1 - pow(e, 2), 3 / 2.0))) *
          (24 * pow(sin(i), 2) * sin(f) *
           ...
           pow(1 + cos(i), 2) *
               (pow(e, 2) *
                ...
                18 * e * sin(2 * g + 2 * h) - 12 * sin(f + 2 * g + 2 * h) +
                28 * sin(3 * f + 2 * g + 2 * h))) +
      ...
      ((J2_MOON * pow(R_MOON, 2)) /
       (32 * pow(a, 2) * e * pow(1 - pow(e, 2), 2))) *
          ((pow(sin(i), 2) *
           ...
           (-8 * e *
            (e * (3 * sin(f + 2 * g) + sin(3 * f + 2 * g) - 6 * sin(f)) -
             6 * f + 6 * l + 3 * sin(2 * f + 2 * g)) *
            pow(cos(i), 2))) +
      ((C22_MOON * pow(R_MOON, 2)) /
       (32 * pow(a, 2) * e * pow(1 - pow(e, 2), 2))) *
          ((24 * pow(sin(i), 2) *
            (sin(f) *
                 (pow(e, 2) * cos(2 * f) + 5 * pow(e, 2) + 6 * e * cos(f) + 6) +
             6 * e * (f - l)) *
            cos(2 * h)) +
           (-8 * e * pow(cos(i), 2) *
            (6 * cos(i) * B_M_1 + (1 - cos(i)) * B_M_2 -
             (1 + cos(i)) * B_M_3)) +
           (pow(1 - cos(i), 2) *
            (-18 * e * sin(2 * g - 2 * h) -
...
std::array<double, 6> ComputeFirstOrderMediumPeriod(ad::Vector6real &coe,
  ...
  double G_mp1 = -15 * k * n3 / 32 * pow(a, 2) * pow(e, 2) *
                 (4 * cos(i) * cos(2 * g) * cos(2 * h) -
                  (cos(2 * i) + 3) * sin(2 * g) * sin(2 * h));
...
std::array<double, 6> ComputeSecondOrderMediumPeriod(ad::Vector6real &coe,
  ...
  double G_mp2 =
      ...
      45 * pow(k, 2) * pow(n3, 2) * pow(a, 7 / 2.0) * pow(e, 2) /
          ...
          (-5 * (-40 * pow(e, 2) + 3 * cos(4 * g) - 3 * cos(4 * h) +
                 28 * cos(2 * i) + 35) +
           4 * cos(2 * i) *
               (5 * (6 * pow(e, 2) - 2 * pow(cos(g), 2) * cos(4 * h) +
                     cos(4 * g)) -
                32 * (pow(e, 2) - 1) * sin(2 * g) * sin(2 * h)) +
           cos(2 * g) *
               (8 * (pow(e, 2) - 1) * (16 * cos(2 * h) - 3) * cos(2 * i) +
                24 * pow(e, 2) + 20 * pow(sin(g), 2) * cos(4 * i) +
                25 * cos(4 * h) - 34) +
  ...
  double H_mp2 =
      ...
      (9 * pow(k, 2) * pow(n3, 2) * pow(a, 3.5) * sqrt(1 - pow(e, 2))) /
          ...
          (-160 * pow(e, 2) * cos(2 * i) * cos(2 * (cos(2 * g) + cos(2 * h))) -
           2 * cos(i) *
               (15 * pow(e, 2) * cos(2 * g) +
                (34 * pow(e, 2) - 4) * cos(2 * h) + 183 * pow(e, 2) + 2) +
           2 * cos(3 * 2 * sin(i)) *
               (15 * pow(e, 2) * cos(2 * g) +
  ...
  double l_mp2 =
      (27 * J2_MOON * C22_MOON * MU_MOON * pow(R_MOON, 4) * pow(sin(i), 2) *
       cos(i)) /
          ...
          sin(2 * h) -
      (9 * J2_MOON * k * pow(R_MOON, 2)) /
          (1024 * pow(a, 2) * pow(1 - pow(e, 2), 2)) *
      ...
      (9 * k * C22_MOON * pow(R_MOON, 2) * pow(sin(i), 2)) /
          ...
          (5 * sin(2 * g) *
           (4 * (pow(e, 2) - 2) * cos(2 * h) * (5 * cos(2 * i) - 1) +
            5 * cos(4 * h) *
                ((pow(e, 2) - 2) * cos(2 * i) + 15 * pow(e, 2) + 6) -
            4 * (10 * (pow(e, 2) + 1) * pow(sin(2 * h), 2) * cos(2 * i) +
                 5 * pow(e, 2) + 11) +
            4 * sin(2 * h) * cos(i) *
  ...
  double g_mp2 =
      ...
      (9 * k * C22_MOON * pow(R_MOON, 2)) /
          ...
          (-8 * (10 * pow(e, 2) * sin(2 * g) * pow(cos(2 * h), 2) *
                     (1 - 5 * cos(2 * i)) +
                 10 * sin(2 * g) * pow(sin(2 * h), 2) * pow(sin(i), 2) *
                     (-6 * pow(e, 2) + 5 * cos(2 * i) + 11) +
                 sin(4 * h) * cos(i) *
                     (10 * cos(2 * g) *
                          ((4 - 5 * pow(e, 2)) * cos(2 * i) + 3 * pow(e, 2) -
                           4) +
                      5 * (3 * pow(e, 2) + 4) * cos(2 * i) + 9 * pow(e, 2) -
                      4)) -
           4 * cos(i) *
               (16 * pow(sin(h), 3) * cos(h) *
                    (5 * (3 * pow(e, 2) + 4) * cos(2 * i) -
                     3 * (pow(e, 2) + 4)) -
      ...
      (9 * pow(k, 2) * pow(n3, 2) * pow(a, 3)) /
          ...
          (10 * sin(2 * g) * cos(4 * h) *
           ...
           (C22_MOON * pow(R_MOON, 2)) /
               (1024 * pow(a, 2) * sqrt(1 - pow(e, 2))) *
               ((1 - pow(e, 2)) * (315 * pow(e, 4) + 160 * pow(e, 2) + 48) *
                    (sin(2 * g) * cos(2 * h) * (cos(2 * i) + 3) +
                     4 * cos(2 * g) * sin(2 * h) * cos(i)) +
                (105 * pow(e, 4) + 80 * pow(e, 2) + 48) * pow(e, 2) *
                    (1 / tan(i)) *
  ...
  double h_mp2 =
      ...
      (9 * k * C22_MOON * pow(R_MOON, 2)) /
          (256 * pow(a, 2) * pow(1 - pow(e, 2), 5 / 2)) *
          (-2 * (5 * pow(e, 2) * sin(2 * g) * cos(i) *
                     (10 * pow(sin(2 * h), 2) * cos(2 * i) + 9 * cos(4 * h) -
                      1) -
...
std::array<double, 6> ComputeCorrectionMediumPeriod(ad::Vector6real &coe,
      ...
      (45 * pow(k, 2) * pow(n3, 2) * pow(a, 3.5) * pow(e, 2)) /
          ...
          (4 * cos(2 * g) * pow(sin(i), 2) *
               (10 * pow(sin(2 * h), 2) * cos(2 * i) - 12 * pow(e, 2) -
                15 * cos(4 * h) + 7) +
           5 * (4 * cos(2 * i) * (-6 * pow(e, 2) + cos(4 * h) + 7) -
                40 * pow(e, 2) + 2 * pow(sin(2 * h), 2) * cos(4 * i) -
                3 * cos(4 * h) + 35) +
  ...
  double l_mp1c =
      (45 * k * C22_MOON * pow(R_MOON, 2) * (5 * pow(e, 2) + 2) *
       pow(sin(i), 2)) /
  ...
  double g_mp1c =
      ...
      (9 * k * C22_MOON * pow(R_MOON, 2) /
       ...
       (40 * sin(2 * g) * pow(cos(2 * h), 2) *
        ...
        5 * sin(2 * g) * pow(sin(2 * h), 2) *
            ((12 - 68 * pow(e, 2)) * cos(2 * i) +
             (5 - 15 * pow(e, 2)) * cos(4 * i) + 19 * pow(e, 2) - 17) +
        2 * sin(4 * h) * cos(i) *
      ...
      (9 * pow(k, 2) * pow(n3, 2) * pow(a, 3) /
       ...
       (-400 * (3 * pow(e, 2) - 2) * pow(sin(g), 3) * cos(g) *
        ...
        8 * sin(4 * h) *
            (cos(i) * (36 * pow(e, 4) - 50 * (2 * pow(e, 2) + 1) * cos(2 * g) +
                       23 * pow(e, 2) -
                       25 * (4 * pow(e, 4) - 21 * pow(e, 2) + 14) * cos(4 * g) +
                       16) -
  ...
  double h_mp1c =
      ...
      (9 * k * C22_MOON * pow(R_MOON, 2) /
       (128 * pow(a, 2) * pow(1 - pow(e, 2), 5.0 / 2.0))) *
          ...
          (40 * pow(e, 2) * sin(2 * g) * cos(i) *
           ...
           sin(4 * h) *
               (25 * pow(e, 4) * cos(4 * g) * (cos(2 * i) + 3) +
                ...
                10 * pow(e, 2) * (13 - 3 * pow(e, 2)) * cos(2 * g) -
                14 * pow(e, 2) +
                (9 * pow(e, 4) + 62 * pow(e, 2) + 4) * cos(2 * i) + 12));
