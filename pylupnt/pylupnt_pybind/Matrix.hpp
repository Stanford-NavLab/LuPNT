 ...
 *
 * Note: need static keyword to avoid multiple definition error
 * https://chunminchang.github.io/blog/post/how-to-avoid-duplicate-symbols-when-compiling
 *
 * @param J
 * @return py::array_t<double>
 */
static py::array_t<double> EigenMatrixToPythonArray(const Eigen::MatrixXd &J) {
  ...
  auto result = py::array_t<double>(
      {J.rows(),
       J.cols()}, /* Pointer to data (nullptr -> ask NumPy to allocate!) */
      {sizeof(double),
       sizeof(double) * J.rows()}, /* Strides (in bytes) for each index */
      foo, free_when_done          /* Pointer to parent */
  );
