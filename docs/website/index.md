<img src='img/lupnt-header.svg' width='100%'>

---

![Linux build status](https://github.com/Stanford-NavLab/LuPNT/workflows/linux/badge.svg?branch=main)
![macOS build status](https://github.com/Stanford-NavLab/LuPNT/workflows/osx/badge.svg?branch=main)
![Windows build status](https://github.com/Stanford-NavLab/LuPNT/workflows/windows/badge.svg?branch=main)

# {{lupnt}}

{{lupnt}} is a C++17 library that uses modern and advanced programming
...
differentiation algorithms implemented in {{lupnt}}: **forward mode** and
**reverse mode**.
...
using {{lupnt}} is relatively simple. For our previous function *f*, we only
need to replace the floating-point type `double` with `lupnt::dual` for both
...
The auxiliary function `lupnt::wrt`, an acronym for **with respect to**,
is used to indicate which input variable *(x, y, z)* is the selected one to
compute the partial derivative of *f*. The auxiliary function `lupnt::at`
...
Similar as before, we can use {{lupnt}} to enable reverse automatic
differentiation for our function *f* by simply replacing type `double` with
`lupnt::var` as follows:
...
The function `lupnt::derivatives` will traverse the expression tree stored
...
and assistance when using {{lupnt}}. If you would like to report a bug, then
