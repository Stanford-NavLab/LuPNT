...
We present here some examples demonstrating the use of {{lupnt}} for
...
suggestions on how to better demonstrate the capabilities of {{lupnt}}.
...
Integrating {{lupnt}} in a CMake-based project is very simple as shown next.
...
to find the header files of the {{lupnt}} library, and the command:

```cmake
target_link_libraries(app lupnt::lupnt)
```
to link the executable target `app` against the {{lupnt}} library
(`lupnt::lupnt`) using CMake's modern target-based design.
...
!!! attention
    ...
    If {{lupnt}} has been installed system-wide, then the CMake argument
    ...
    assuming directory `$HOME/local` is where {{lupnt}} was installed to, which
