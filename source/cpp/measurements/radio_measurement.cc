...
Real RadioMeasurement::ComputeOneWayRangeLTR(
    Real epoch_rx, Vec6 rv_tx, Vec6 rv_rx, Real dt_tx, Real dt_rx,
    std::shared_ptr<Body> tx_center_body, std::shared_ptr<Body> rx_center_body,
    bool is_bodyfixed_tx, bool is_bodyfixed_rx, Real hardware_delay) {
  ...
  for (int i = 0; i < max_iter; i++) {
    ...
    if (!is_bodyfixed_tx) {  // is not ground station
      rid_p = ri - vi * (tau_d_rx + tau_d) +
              1 / 2 * ai_ri * pow(tau_d_rx + tau_d, 2);
    } else {
      // Fixed to surface frame -> convert to ITRF frame
...
Real RadioMeasurement::ComputeTwoWayRangeLTR(
    ...
    bool is_bodyfixed_rx, Real hardware_delay, Real additional_delay) {
  // hardware delays
  Real tau_d_rx = hardware_delay;  // receiver delay for downlink (xi->x0)
  ...
  Real rho_d = ComputeOneWayRangeLTR(epoch_rx, rv_target_tr, rv_rx_tr, 0.0, 0.0,
                                     target_center_body, rx_center_body,
                                     is_bodyfixed_target, is_bodyfixed_rx,
                                     tau_d_rx + additional_delay);
  Real tau_d = rho_d / C;

  // solve for tau_u (uplink time, rx->target)
  Real tau_c_pp = tau_u_rx + tau_d_tx + tau_d_rx;  // total hardware delay
  Real delay_uplink =
      tau_c_pp + tau_d;  // total delay for uplink w.r.t epoch_rx
  Real rho_u = ComputeOneWayRangeLTR(epoch_rx, rv_rx_tr, rv_target_tr, 0.0, 0.0,
                                     rx_center_body, target_center_body,
                                     is_bodyfixed_rx, is_bodyfixed_target,
...
Real RadioMeasurement::ComputeOneWayRangeRateLTR(
    Real epoch_rx, Vec6 rv_tx_tr, Vec6 rv_rx_tr, Real dt_dot_tx, Real dt_dot_rx,
    std::shared_ptr<Body> target_center_body,
    std::shared_ptr<Body> rx_center_body, bool is_bodyfixed_target,
    bool is_bodyfixed_rx, Real hardware_delay, double T_I) {
  Real rho_d = ComputeOneWayRangeLTR(
      epoch_rx, rv_tx_tr, rv_rx_tr, 0.0, 0.0, target_center_body,
      rx_center_body, is_bodyfixed_target, is_bodyfixed_rx, hardware_delay);
  Real rho_d_past = ComputeOneWayRangeLTR(
      epoch_rx, rv_tx_tr, rv_rx_tr, 0.0, 0.0, target_center_body,
...
Real RadioMeasurement::ComputeTwoWayRangeRateLTR(
    Real epoch_rx, Vec6 rv_target_tr, Vec6 rv_rx_tr,
    std::shared_ptr<Body> target_center_body,
    std::shared_ptr<Body> rx_center_body, bool is_bodyfixed_target,
    bool is_bodyfixed_rx, Real hardware_delay, double T_I) {
  Real rho_ud = ComputeTwoWayRangeLTR(
      epoch_rx, rv_target_tr, rv_rx_tr, target_center_body, rx_center_body,
      is_bodyfixed_target, is_bodyfixed_rx, hardware_delay);
  Real rho_ud_past = ComputeTwoWayRangeLTR(
      epoch_rx, rv_target_tr, rv_rx_tr, target_center_body, rx_center_body,
      is_bodyfixed_target, is_bodyfixed_rx, hardware_delay, T_I);
