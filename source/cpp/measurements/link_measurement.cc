...
LinkMeasurement::LinkMeasurement(std::vector<NaifId> occult_bodies,
                                 VecXd occult_alt, Real hardware_delay)
...
void LinkMeasurement::SetLinkParams() {
  ...
  // Link Parameters
  Body tx_center_body = CreateDefaultBody(trans.tx->GetAgent()->GetBodyId());
  Body rx_center_body = CreateDefaultBody(trans.rx->GetAgent()->GetBodyId());
  linkparams_.tx_center_body = std::make_shared<Body>(tx_center_body);
  linkparams_.rx_center_body = std::make_shared<Body>(rx_center_body);
...
void LinkMeasurement::GenerateOneWayLink(Real epoch,
...
VecX LinkMeasurement::GetTrueOneWayLinkMeasurement(
  ...
  return GetOneWayLinkMeasurement(epoch_rx_true_, rv_tx, rv_rx, clk_tx, clk_rx,
                                  H_ow_rx, hardware_delay_, meas_types, true,
                                  false);
}

VecX LinkMeasurement::GetOneWayLinkMeasurement(
  ...
  for (auto meas_type : meas_types) {
    switch (meas_type) {
      case LinkMeasurementType::Range:
        rho_ow = GetOneWayRangeMeasurement(epoch_rx, rv_tx, rv_rx, clk_tx,
                                           clk_rx, H_ow_range, hardware_delay,
      ...
      case LinkMeasurementType::RangeRate:
        rho_ow_rate = GetOneWayRangeRateMeasurement(
            epoch_rx, rv_tx, rv_rx, clk_tx, clk_rx, H_ow_rangerate,
...
Real LinkMeasurement::GetOneWayRangeMeasurement(
    Real epoch_rx, Vec6 rv_tx, Vec6 rv_rx, Vec2 clk_tx, Vec2 clk_rx,
    MatXd &H_ow_rx, Real hardware_delay, bool with_noise, bool with_jacobian) {
  // return

  auto func = [epoch_rx, hardware_delay, this](
                  const Vec6 rv_tx_in, const Vec2 clk_tx_in,
                  const Vec6 rv_rx_in, const Vec2 clk_rx_in) {
    Real owr = RadioMeasurement::ComputeOneWayRangeLTR(
        epoch_rx, rv_tx_in, rv_rx_in, clk_tx_in(0), clk_rx_in(0),
  ...
  if (with_jacobian) {
    VecXd H_ow_vec =
        gradient(func, wrt(rv_tx_tmp, clk_tx_tmp, rv_rx_tmp, clk_rx_tmp),
...
Real LinkMeasurement::GetOneWayRangeRateMeasurement(
    Real epoch_rx, Vec6 rv_tx, Vec6 rv_rx, Vec2 clk_tx, Vec2 clk_rx,
    MatXd &H_ow_rx, Real hardware_delay, bool with_noise, bool with_jacobian) {
  auto func = [epoch_rx, rv_tx, clk_tx, hardware_delay, this](
    ...
    Real owrr = RadioMeasurement::ComputeOneWayRangeRateLTR(
        epoch_rx, rv_tx_in, rv_rx_in, clk_tx(1), clk_rx_in(1),
        linkparams_.tx_center_body, linkparams_.rx_center_body,
        linkparams_.is_bodyfixed_tx, linkparams_.is_bodyfixed_rx,
...
void LinkMeasurement::GenerateTwoWayLink(
  ...
  if (txrx == "rx") {
    t_rx_d = epoch;
    trans_d =
        sc.ComputeLinkBudget(tx_d, rx_d, t_rx_d - delay_rx_receiver, "rx");
    ...
    t_tx_u = epoch;
    trans_u =
        sc.ComputeLinkBudget(tx_u, rx_u, t_tx_u + delay_tx_receiver, "tx");
...
VecX LinkMeasurement::GetTrueTwoWayLinkMeasurement(
    std::vector<LinkMeasurementType> meas_types) {
  ...
  return GetTwoWayLinkMeasurement(epoch_rx_true_, rv_receiver, rv_target,
                                  H_tw_rx, hardware_delay_, meas_types, true,
                                  false);
}

VecX LinkMeasurement::GetTwoWayLinkMeasurement(
    Real epoch_rx, Vec6 rv_receiver, Vec6 rv_target, MatXd H_tw_rx,
  ...
  for (auto meas_type : meas_types) {
    switch (meas_type) {
      ...
      case LinkMeasurementType::RangeRate:
        rho_tw_rate = GetTwoWayRangeRateMeasurement(
            epoch_rx, rv_receiver, rv_target, H_tw_rangerate, hardware_delay,
            with_noise, with_jacobian);
        z(idx) = rho_tw_rate;
...
Real LinkMeasurement::GetTwoWayRangeMeasurement(
    Real epoch_rx, Vec6 rv_receiver, Vec6 rv_target, MatXd &H_tw_range,
    Real hardware_delay, bool with_noise, bool with_jacobian) {
  // return

  auto func = [epoch_rx, hardware_delay, this](const Vec6 rv_target_in,
                                               const Vec6 rv_receiver_in) {
    Real twr = RadioMeasurement::ComputeTwoWayRangeLTR(
        epoch_rx, rv_target_in, rv_receiver_in, linkparams_.tx_center_body,
        linkparams_.rx_center_body, linkparams_.is_bodyfixed_tx,
...
Real LinkMeasurement::GetTwoWayRangeRateMeasurement(
    Real epoch_rx, Vec6 rv_receiver, Vec6 rv_target, MatXd &H_tw_rr,
    Real hardware_delay, bool with_noise, bool with_jacobian) {
  // Function to compute the two way range rate
  auto func = [epoch_rx, rv_target, hardware_delay, this](
                  const Vec6 rv_target_in, const Vec6 rv_receiver_in) {
    Real owrr = RadioMeasurement::ComputeTwoWayRangeRateLTR(
        epoch_rx, rv_target_in, rv_receiver_in, linkparams_.tx_center_body,
        linkparams_.rx_center_body, linkparams_.is_bodyfixed_tx,
        linkparams_.is_bodyfixed_rx, hardware_delay, linkparams_.T_I_doppler);
    return owrr;
  };
