...
std::map<std::string, bool> Occultation::ComputeOccultation(
  ...
  assert(bodies.size() == atm_h.size() &&
         "bodies and atm_h must have the same size");
  ...
  for (size_t i = 0; i < bodies.size(); i++) {
    ...
    Vec3d rb = GetBodyPosVel(epoch, NaifId::SOLAR_SYSTEM_BARYCENTER, bodies[i])
                   .head(3)
                   .cast<double>();
    ...
    // cos(beta))
    bool is_vis =
        (alpha_body < beta_body) && (r12_norm > r1b_norm * cos(beta_body));
...
std::vector<std::map<std::string, bool>> Occultation::ComputeOccultation(
  ...
  for (int i = 0; i < std::max(r1.rows(), r2.rows()); i++) {
    ...
    vis.push_back(
        ComputeOccultation(epoch, r1_vec, r2_vec, cs1, cs2, bodies, atm_h));
  }
  return vis;
}
...
std::vector<std::map<std::string, bool>> Occultation::ComputeOccultation(
  ...
  for (int i = 0; i < epoch.size(); i++) {
    ...
    Vec3 r2_vec = r2.rows() == 1 ? r2.row(0) : r2.row(i);
    vis.push_back(
        ComputeOccultation(epoch(i), r1_vec, r2_vec, cs1, cs2, bodies, atm_h));
  }
  return vis;
}
